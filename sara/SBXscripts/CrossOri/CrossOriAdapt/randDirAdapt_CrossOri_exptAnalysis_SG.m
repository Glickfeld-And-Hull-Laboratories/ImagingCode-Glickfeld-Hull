...
adptgrat_dir = [1]; % grating direction of adaptor stim (stimOneGratingDirection)
adptpatt_dir = [3 11]; % pattern directions where one of the components is the adaptor direction (i.e., adapt dir +/- 60)

pref_gratadptdir = find(maxDir==adptgrat_dir); % neurons that prefer the grating direction that is adapted
pref_pattadptdir = [find(maxDir==adptpatt_dir(1)); find(maxDir==adptpatt_dir(2))]; % neurons that prefer grating directions that will be the adapted pattern direction
...
figure;
    ...
    subplot(3,2,2)
        ...
        subtitle('Zp')
    subplot(3,2,3)
        scatter(Zc(1,pref_gratadptdir),Zc(2,pref_gratadptdir))
        hold on
        refline(1)
        subtitle('Zc')
    subplot(3,2,4)
        scatter(Zp(1,pref_gratadptdir),Zp(2,pref_gratadptdir))
        hold on
        refline(1)
        subtitle('Zp')
    subplot(3,2,5)
        scatter(Zc(1,pref_pattadptdir),Zc(2,pref_pattadptdir))
        hold on
        refline(1)
        subtitle('Zc')
    subplot(3,2,6)
        scatter(Zp(1,pref_pattadptdir),Zp(2,pref_pattadptdir))
        hold on
        refline(1)
        subtitle('Zp')
