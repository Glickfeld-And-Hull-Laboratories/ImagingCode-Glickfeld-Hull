...
[n n2] = subplotn(nep);
f = figure; for i = 1:nep; subplot(n,n2,i); imagesc(mean(data(:,:,1+((i-1)*regIntv):nframes_avg+((i-1)*regIntv)),3)); title([num2str(1+((i-1)*regIntv)) '-' num2str(nframes_avg+((i-1)*regIntv))]); colormap gray; clim([0 3000]); end
movegui('center')
sgtitle('Click on a stable, sharp epoch for registration reference')

% Interactive selection - click on desired subplot
...
fprintf(['Selected epoch ' num2str(numClicked) '\n'])

% Calculate reference frame range from selection
refStart = 1 + ((numClicked-1) * regIntv);
refEnd = nframes_avg + ((numClicked-1) * regIntv);
fprintf(['Using frames ' num2str(refStart) ':' num2str(refEnd) ' as reference\n'])

%% Register data
data_avg = mean(data(:,:,refStart:refEnd),3);
...
stimTFs = unique(stimTF_all);
nStimTF = length(stimTFs);
