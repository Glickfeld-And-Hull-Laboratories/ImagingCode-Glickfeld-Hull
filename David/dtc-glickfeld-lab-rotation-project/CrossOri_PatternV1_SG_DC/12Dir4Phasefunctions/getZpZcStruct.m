...
function [ZpZcStruct] = getZpZcStruct(avg_resp_dir, data_type, plaid_offset_deg)
...
% Create pattern and component predictions
    if strcmp(data_type, 'alignedTestDir')
        component   = avg_resp_dir(:,:,1,1,1)+circshift(avg_resp_dir(:,:,1,1,1),-plaid_offset_deg./int,2); % component prediction, shift grating resp by plaid angle and add
        pattern     = circshift(avg_resp_dir(:,:,1,1,1),-half_offset./int,2);    % pattern prediction, shift grating resp by half plaid angle
    elseif strcmp(data_type, 'whole_cell')
        component = circshift(avg_resp_dir(:,:,1,1,1),+half_offset./int,2)+circshift(avg_resp_dir(:,:,1,1,1),-half_offset./int,2);
...
% Find PDS and CDS cells
    ...
    PDSind_byphase = cell(1, nMaskPhas);   % Initialize cell array for lists of PDS cell indices
    CDSind_byphase = cell(1, nMaskPhas);
