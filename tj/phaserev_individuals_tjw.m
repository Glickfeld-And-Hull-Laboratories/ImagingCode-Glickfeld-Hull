...
fnout = '\\duhs-user-nc1.dhe.duke.edu\dusom_glickfeldlab\All_Staff\home\tj\Analysis\Analysis\2P\SRP_segmented'; %folder to load files from
newfnout = '\\duhs-user-nc1.dhe.duke.edu\dusom_glickfeldlab\All_Staff\home\tj\Analysis\Analysis\2P\phaserev\updated'; %folder to save files to
...
d1 = 2+7; %base1 in expt list
d2 = 3+7; %base1+4d in expt list
d3 = 4+7;
d4 = 5+7;
d5 = 6+7;
...
d1_avgs = load(fullfile(fnout, [date_d1 '_' mouse], [date_d1 '_' mouse '_' ref_str_d1], [date_d1 '_' mouse '_' ref_str_d1 '_' 'keyvars.mat']));
...
%load max
d1_maxes = load(fullfile(fnout, [date_d1 '_' mouse], [date_d1 '_' mouse '_' ref_str_d1], [date_d1 '_' mouse '_' ref_str_d1 '_' 'segmentData.mat']));
d2_maxes = load(fullfile(fnout, [date_d2 '_' mouse], [date_d2 '_' mouse '_' ref_str_d2], [date_d2 '_' mouse '_' ref_str_d2 '_' 'segmentData.mat']));
d3_maxes = load(fullfile(fnout, [date_d3 '_' mouse], [date_d3 '_' mouse '_' ref_str_d3], [date_d3 '_' mouse '_' ref_str_d3 '_' 'segmentData.mat']));
d4_maxes = load(fullfile(fnout, [date_d4 '_' mouse], [date_d4 '_' mouse '_' ref_str_d4], [date_d4 '_' mouse '_' ref_str_d4 '_' 'segmentData.mat']));
d5_maxes = load(fullfile(fnout, [date_d5 '_' mouse], [date_d5 '_' mouse '_' ref_str_d5], [date_d5 '_' mouse '_' ref_str_d5 '_' 'segmentData.mat']));
...
load(fullfile(fnout, [date_d1 '_' mouse], [date_d1 '_' mouse '_' ref_str_d1], [date_d1 '_' mouse '_' ref_str_d1 '_' 'input.mat']));


%%
avg_pop_resp_all = [d1_avgs.avg_pop_resp d2_avgs.avg_pop_resp d3_avgs.avg_pop_resp d4_avgs.avg_pop_resp d5_avgs.avg_pop_resp];
se_pop_resp_all = [d1_avgs.se_pop_resp d2_avgs.se_pop_resp d3_avgs.se_pop_resp d4_avgs.se_pop_resp d5_avgs.se_pop_resp];

figure;
a=errorbar(avg_pop_resp_all, se_pop_resp_all);
...
%%
d1_grand_avg = mean(d1_avgs.avg_pop_resp);
d1_grand_se = std(d1_avgs.se_pop_resp)./sqrt(size(d1_avgs.avg_pop_resp,1));

d2_grand_avg = mean(d2_avgs.avg_pop_resp);
d2_grand_se = std(d2_avgs.se_pop_resp)./sqrt(size(d2_avgs.avg_pop_resp,1));

d3_grand_avg = mean(d3_avgs.avg_pop_resp);
d3_grand_se = std(d3_avgs.se_pop_resp)./sqrt(size(d3_avgs.avg_pop_resp,1));

d4_grand_avg = mean(d4_avgs.avg_pop_resp);
d4_grand_se = std(d4_avgs.se_pop_resp)./sqrt(size(d4_avgs.avg_pop_resp,1));

d5_grand_avg = mean(d5_avgs.avg_pop_resp);
d5_grand_se = std(d5_avgs.se_pop_resp)./sqrt(size(d5_avgs.avg_pop_resp,1));


figure;
errorbar([d1_grand_avg d2_grand_avg d3_grand_avg d4_grand_avg d5_grand_avg], [d1_grand_se d2_grand_se d3_grand_se d4_grand_se d5_grand_se])
xlim([0 6])
ylim([0 .2])
ylabel('Avg dF/F')
xticklabels({'','Day 1 SRP','Day 2 SRP','Day 3 SRP','Day 4 SRP','Day 5 SRP', ''})
print(fullfile(newfnout, [mouse,  '_grand_avg_across_days.pdf']), '-dpdf', '-bestfit')

%%

all_n_sig_cells = [d1_avgs.n_sig_cell d2_avgs.n_sig_cell d3_avgs.n_sig_cell d4_avgs.n_sig_cell d5_avgs.n_sig_cell];

figure;
bar(all_n_sig_cells);
ylabel('# of visually responsive cells')
ylim([0 100])
xticklabels({'Day 1 SRP','Day 2 SRP','Day 3 SRP','Day 4 SRP','Day 5 SRP'})
print(fullfile(newfnout, [mouse,  '_n_sig_cells.pdf']), '-dpdf', '-bestfit')


%%
figure;
a = cdfplot(d1_avgs.avg_pop_resp);
set(a, 'Color', [0 0 0], 'LineWidth', 2)
hold on;
b = cdfplot(d2_avgs.avg_pop_resp);
set(b, 'Color', [.25 .25 .25], 'LineWidth', 2)
c = cdfplot(d3_avgs.avg_pop_resp);
set(c, 'Color', [.5 .5 .5], 'LineWidth', 2)
d = cdfplot(d4_avgs.avg_pop_resp);
set(d, 'Color', [.75 .75 .75], 'LineWidth', 2)
e = cdfplot(d5_avgs.avg_pop_resp);
set(e, 'Color', [.9 .9 .9], 'LineWidth', 2)
xlim([0 .5])
xlabel('Avg dF/F')
ylabel('% of cells')
legend(['Day 1'], ['Day 2'], ['Day 3'], ['Day 4'], ['Day 5'])
title(mouse)
print(fullfile(newfnout, [mouse,  '_avgdfof_cdf.pdf']), '-dpdf', '-bestfit')
...
a=subplot(2,3,1)
plot(mean(mean(d1_avgs.data_dfof,2),3))
ylim([-.05 .3])
vline(75)
b=subplot(2,3,2)
plot(mean(mean(d2_avgs.data_dfof,2),3))
ylim([-.05 .3])
vline(75)
c=subplot(2,3,3)
plot(mean(mean(d3_avgs.data_dfof,2),3))
ylim([-.05 .3])
vline(75)
d=subplot(2,3,4.5)
plot(mean(mean(d4_avgs.data_dfof,2),3))
ylim([-.05 .3])
vline(75)
e=subplot(2,3,5.5)
plot(mean(mean(d5_avgs.data_dfof,2),3))
ylim([-.05 .3])
vline(75)
...
linkaxes([a b c d e],'xy')
ylabel(han,'Avg dF/F');
xlabel(han,'Frame');
title('Significantly Responsive Cells')

print(fullfile(newfnout, [mouse,  '_sig_cell_across_frames.pdf']), '-dpdf', '-bestfit')
...
sgtitle('Max dF/F')
subplot(2,3,1)
imagesc(d1_maxes.data_dfof_max)
clim([-.45 1.18])

subplot(2,3,2)
imagesc(d2_maxes.data_dfof_max)
clim([-.45 1.18])

subplot(2,3,3)
imagesc(d3_maxes.data_dfof_max)
clim([-.45 1.18])

subplot(2,3,4.5)
imagesc(d4_maxes.data_dfof_max)
clim([-.45 1.18])

subplot(2,3,5.5)
imagesc(d5_maxes.data_dfof_max)
clim([-.45 1.18])

print(fullfile(newfnout, [mouse,  '_max_dfof_fovs.pdf']), '-dpdf', '-bestfit')

%%

[n1 n2] = subplotn(d1_avgs.n_sig_cell);
figure;
for i = 1:d1_avgs.n_sig_cell
    subplot(n1,n2,i);
    plot(mean(d1_avgs.data_dfof(:,i,:),3))
end
print(fullfile(newfnout, [mouse, '_', date_d1, '_single_cell_tcs.pdf']), '-dpdf', '-fillpage')

[n1 n2] = subplotn(d2_avgs.n_sig_cell);
figure;
for i = 1:d2_avgs.n_sig_cell
    subplot(n1,n2,i);
    plot(mean(d2_avgs.data_dfof(:,i,:),3))
end
print(fullfile(newfnout, [mouse, '_', date_d2, '_single_cell_tcs.pdf']), '-dpdf', '-fillpage')

[n1 n2] = subplotn(d3_avgs.n_sig_cell);
figure;
for i = 1:d3_avgs.n_sig_cell
    subplot(n1,n2,i);
    plot(mean(d3_avgs.data_dfof(:,i,:),3))
end
print(fullfile(newfnout, [mouse, '_', date_d3, '_single_cell_tcs.pdf']), '-dpdf', '-fillpage')

[n1 n2] = subplotn(d4_avgs.n_sig_cell);
figure;
for i = 1:d4_avgs.n_sig_cell
    subplot(n1,n2,i);
    plot(mean(d4_avgs.data_dfof(:,i,:),3))
end
print(fullfile(newfnout, [mouse, '_', date_d4, '_single_cell_tcs.pdf']), '-dpdf', '-fillpage')

[n1 n2] = subplotn(d5_avgs.n_sig_cell);
figure;
for i = 1:d5_avgs.n_sig_cell
    subplot(n1,n2,i);
    plot(mean(d5_avgs.data_dfof(:,i,:),3))
end
print(fullfile(newfnout, [mouse, '_', date_d5, '_single_cell_tcs.pdf']), '-dpdf', '-fillpage')
