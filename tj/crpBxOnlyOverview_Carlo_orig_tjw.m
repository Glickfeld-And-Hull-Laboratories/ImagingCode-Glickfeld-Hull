%% Modified version of the analysis overview for analyzing the licking behavior of cue-reward trials. 
...
%this part was deleting all folders under "Analysis" when crp was added to
%the end of it
bxSourceBase = '\\duhs-user-nc1.dhe.duke.edu\dusom_glickfeldlab\All_Staff\home\tj\crp_data'; %base folder for data from bx session
bxOutputBase = '\\duhs-user-nc1.dhe.duke.edu\dusom_glickfeldlab\All_Staff\home\tj\test\Behavior\crp'; %base folder for output for bx analysis
crpFigBase = '\\duhs-user-nc1.dhe.duke.edu\dusom_glickfeldlab\All_Staff\home\tj\test\Behavior\crp_figs'; %base folder for output for figures

% bxSourceBase = '\\duhs-user-nc1.dhe.duke.edu\dusom_glickfeldlab\All_Staff\home\tj\crp_data'; %base folder for data from bx session
% bxOutputBase = '\\duhs-user-nc1.dhe.duke.edu\dusom_glickfeldlab\All_Staff\home\tj\Analysis\Behavior'; %base folder for output for bx analysis
% crpFigBase = '\\duhs-user-nc1.dhe.duke.edu\dusom_glickfeldlab\All_Staff\home\tj\Analysis\Behavior\crp_figs'; %base folder for output for figures
...
crpTrainDayList_tjw; %load blank variable arrays
...
[thisMouse, thisDate] = select_mouse_info(days{1}); 
...
%gapInx = gapDayFinder(days);
    split = input('Split trial licks into quartiles?: ');
...
for ii = 1:length(days)
    ...
    bxData = get_bx_data_tj(bxSourceBase, days{ii});  %find the correct behavior file and loads it.
        %pulls out mouse ID and date of bx session
    [thisMouse, thisDate] = select_mouse_info(days{ii});
    ...
    %% gonna grab information about ITI length, frame numbers, and reward size for excel sheet if any info was missed on the day
    ITIlength(1,ii) = bxData.itiRangeMaxTimeMs;
    ITIlength(2,ii) = bxData.itiRangeMinTimeMs;
    framelength(ii,1) = bxData.counterValues{1,end}(1,end);
    seshlength(ii,1) = length(bxData.tBlock2TrialNumber);
    rewardsize(1,ii) = bxData.minRewardUs;
    propCSP(ii,1) = sum(~cell2mat(bxData.tBlock2TrialNumber))./size(cell2mat(bxData.tBlock2TrialNumber),2);
    %%
    ...
    if bxData.doBlock2 == 1
        ...
        if exist([sessionFigs, days{ii}, '_both_cum_hist.fig'], 'file')==0
            ...
            saveas(tempFig, [sessionFigs days{ii} '_bothLickHist.pdf']);
    ...
    if exist([sessionFigs, days{ii}, '_rew_lick_raster.fig'], 'file')==0
        ...
        %(:,1500:3500) for publication figs
        tempFig=setFigure_tj;
        %axis([-1*timeBeforeCue timeAfterCue 0 72]);
        plotSpikeRaster(logical(licks_by_trial_rewarded), 'PlotType', 'vertline', 'XLimForCell', [-500 1500]); %can't align cue to 0? adding 'x_axis_range' breaks command 
        ...
        saveas(tempFig, [sessionFigs days{ii} 'rewLickRaster.pdf']);
    end
    ...
    if bxData.doBlock2 == 1
        if exist([sessionFigs, days{ii}, '_block2_lick_raster.fig'], 'file')==0
            ...
            tempFig=setFigure_tj;
            plotSpikeRaster(logical(licks_by_trial_block2), 'PlotType', 'vertline', 'XLimForCell', [-500 1500]);
            vline(timeBeforeCue+1, 'k');
            vline(timeBeforeCue+1 + cue_rew_int, 'b');
            ylabel('trial # (descending)');
            ...
            saveas(tempFig, [sessionFigs days{ii} 'block2LickRaster.pdf']);
        end
...
%     end
    ...
    for kk = 1:numTrials-1 %look at each trial excluding the last
         ...
         elseif ~isempty(this_trial_post_cue_licks) %Use first lick in place of bursting if 1<=#licks<=2
             this_trial_RT = this_trial_post_cue_licks(1);
             RT_this_session = [RT_this_session, this_trial_RT]; %store the RTs
         else %if no licks were observed - ***coding this as 5000 to indicate the animal's reaction was at the maximum time allotted
             %RT_this_session = [RT_this_session, NaN]; 
             RT_this_session = [RT_this_session, 5000];
         end
    end
    ...
    %save([crpFigBase, 'rtData\' , days{ii}], 'RT_this_session_raw', 'timeBeforeCue');  %***this is saved in crpRTdata folder under date_mouse_time
    ...
    save([crpFigBase, 'rtData\' , days{ii}], 'RT_this_session_raw', 'timeBeforeCue', 'RT_this_session', 'RT_this_session_block2');  %***this is saved in crpRTdata folder under date_mouse_time
...
%***changing this var to be for hits only RT rather than all trials
...
%     elseif rtTrimToggle ***comment this out
% %changed from RT_this_session>1400 to RT_this_session>1500
%          miss_rate_this_session = (length(find(RT_this_session>1200))+no_lick_trials) / num_trials_b1;
% %changed from 200<RT_this_session<500 to 500<RT_this_session<1500
%          RT_this_session = RT_this_session(RT_this_session>200 & RT_this_session<1200); 
%         if  bxData.doBlock2 ==1
% %changed from RT_this_session>1400 to RT_this_session>1500            
%             miss_rate_this_session_b2 = (length(find(RT_this_session_block2>1200))+no_lick_trials_b2) / num_trials_b2;
% %changed from 200<RT_this_session<500 to 500<RT_this_session<1500
%             RT_this_session_block2 = RT_this_session_block2(RT_this_session_block2>200 & RT_this_session_block2<1200);
%         end
    end
    
    %%

    %Determine the FA and miss rates for this session. Store stats for
    %summary statistics **added info for hits only here
    hit_RT_across_days = [];
    hit_RT_across_days_sem = [];
    hit_std_of_RT_across_days = [];
    ...
    hit_RT_across_days = [hit_RT_across_days, mean(hit_RT_this_session)]; %values plotted in summary graphs
    hit_RT_across_days_sem = [hit_RT_across_days_sem, std(hit_RT_this_session)/sqrt(size(hit_RT_this_session,2))];
    hit_std_of_RT_across_days = [hit_std_of_RT_across_days, std(hit_RT_this_session)];
    %block2
    if bxData.doBlock2 ==1
        if rtTrimToggle
    ...
    if exist([sessionFigs, days{ii}, 'raw_RT_plot.fig'], 'file')==0
        tempFig=setFigure_tj; plot(RT_this_session);
        title(['Raw RT for CS+ trials within for day ' thisDate, thisMouse]);
        xlabel('trial #');
        ylabel('RT (ms)');
        savefig([sessionFigs, days{ii}, 'raw_RT_plot']);
    end
    %plot block2 RT
    if bxData.doBlock2 ==1
        if exist([sessionFigs, days{ii}, 'raw_RT_plot_block2.fig'], 'file')==0;
            tempFig=setFigure_tj; plot(RT_this_session_block2);
            title(['Raw RT for CS- trials within for day ' thisDate, ' img' thisMouse]);
            xlabel('trial #');
            ylabel('RT (ms)');
            savefig([sessionFigs, days{ii}, 'raw_RT_plot_block2']);
        end
    end
%***plot for hits only
    if exist([sessionFigs, days{ii}, 'hit_RT_plot.fig'], 'file')==0
        tempFig=setFigure_tj; plot(hit_RT_this_session);
        title(['Hit RT for CS+ trials within for day ' thisDate, thisMouse]);
        xlabel('trial #');
        ylabel('RT (ms)');
        savefig([sessionFigs, days{ii}, 'hit_RT_plot']);
    end
    %plot block2 RT
    if bxData.doBlock2 ==1
        if exist([sessionFigs, days{ii}, 'hit_RT_plot_block2.fig'], 'file')==0;
            tempFig=setFigure_tj; plot(hit_RT_this_session_block2);
            title(['Hit RT for CS- trials within for day ' thisDate, ' img' thisMouse]);
            xlabel('trial #');
            ylabel('RT (ms)');
            savefig([sessionFigs, days{ii}, 'hit_RT_plot_block2']);
        end
    end
    
    %%
    
    hit_RT_across_sessions = [];
    hit_RT_across_sessions_b2 = [];
    RT_across_sessions_b2 = [];

    RT_across_sessions = [RT_across_sessions, RT_this_session];
    hit_RT_across_sessions = [hit_RT_across_sessions, hit_RT_this_session];
    RT_across_sessions_b2 = [RT_across_sessions_b2, RT_this_session_block2];
    hit_RT_across_sessions_b2 = [hit_RT_across_sessions_b2, hit_RT_this_session_block2];
...
%     if bxData.rewardDelayPercent == 0 && bxData.tooFastTimeMs > 0
%         RT_across_sessions_delay = [RT_across_sessions_delay, RT_this_session];
%         if bxData.doBlock2 == 1
%             RT_across_sessions_delay_b2 = [RT_across_sessions_delay_b2, RT_this_session_block2];
%         end
...
%         end
    ...
    full_trial_licks_omit_ste = std(full_trial_licks_omit,1)./sqrt(length(full_trial_licks_omit));
    full_trial_licks_rewarded_bin_ste = zeros(1,(length(full_trial_licks_rewarded_ste)/bin_size));
%     full_trial_licks_rewardedFirstQuart_bin_ste = zeros(1,(length(full_trial_licks_rewarded_ste)/bin_size));
%     full_trial_licks_rewardedSecondQuart_bin_ste = zeros(1,(length(full_trial_licks_rewarded_ste)/bin_size));
...
%     full_trial_licks_rewardedFirstQuart_bin = zeros(1,(length(full_trial_licks_rewarded_sum)/bin_size));
%     full_trial_licks_rewardedSecondQuart_bin = zeros(1,(length(full_trial_licks_rewarded_sum)/bin_size));
    full_trial_licks_omit_bin = zeros(1,(length(full_trial_licks_omit_sum)/bin_size));
    full_trial_licks_omit_bin_ste = zeros(1,(length(full_trial_licks_omit_ste)/bin_size));
...
%         full_trial_licks_block2FirstQuart_ste = std(full_trial_licks_block2,1)./sqrt(length(full_trial_licks_block2));
%         full_trial_licks_block2SecondQuart_ste = std(full_trial_licks_block2,1)./sqrt(length(full_trial_licks_block2));
        full_trial_licks_block2_bin = zeros(1,(length(full_trial_licks_block2_sum)/bin_size));
%         full_trial_licks_block2_binFirstQuart = zeros(1,(length(full_trial_licks_block2_sum)/bin_size));
%         full_trial_licks_block2_binSecondQuart = zeros(1,(length(full_trial_licks_block2_sum)/bin_size));
    ...
    full_trial_licks_omit_bin_ste = (full_trial_licks_omit_bin_ste/size(full_trial_licks_omit_ste,1))*(1000/bin_size); % convert to lick rate in Hz
    ...
    if exist([sessionFigs, days{ii}, '_rew_lick_hist.fig'], 'file')==0 
        tempFig=setFigure_tj; bar(x_axis_bin, full_trial_licks_rewarded_bin);
    ...
    if bxData.rewardOmissionPercent >0
        if exist([sessionFigs, days{ii}, '_om_lick_hist.fig'], 'file')==0
            tempFig=setFigure_tj; bar(x_axis_bin, full_trial_licks_omit_bin);
    ...
    if bxData.rewardUnexpectPercent >0
        if exist([sessionFigs, days{ii}, '_unexp_lick_hist.fig'], 'file')==0;
            tempFig=setFigure_tj; bar(x_axis_bin, full_trial_licks_unexp_bin);
    ...
    if bxData.doBlock2 > 0
        ...
        if exist([sessionFigs, days{ii}, '_block2_lick_hist.fig'], 'file')==0
            tempFig=setFigure_tj; bar(x_axis_bin, full_trial_licks_block2_bin);
...
tempFig=setFigure_tj; 
bar(x_axis_bin, full_trial_licks_rewarded_bin, 'black'); hold on;
...
saveas(tempFig, [sessionFigs days{ii} '_stackedLickHistSEM.pdf']);
    end
...
axis([-1 4 0 10]); xticks([-1 0 1 2 3 4]); yticks([0 5 10]);
xlim([-1 5])
savefig([sessionFigs thisDate '_img' thisMouse '_firstQuartStackedLickHistSEM']);
saveas(tempFig, [sessionFigs days{ii} '_firstQuartStackedLickHistSEM.pdf']);
...
axis([-1 4 0 10]); xticks([-1 0 1 2 3 4]); yticks([0 5 10]);
xlim([-1 5])
savefig([sessionFigs thisDate '_img' thisMouse '_lastQuartStackedLickHistSEM']);
saveas(tempFig, [sessionFigs days{ii} '_lastQuartStackedLickHistSEM.pdf']);
...
if bxData.rewardOmissionPercent > 1
    tempFig=setFigure_tj; subplot(1,2,2); 
...
title('Miss rates as a % of all trials by day: misses = RT>720ms');
if bxData.doBlock2 ==1
...
if length(days_divider_inx) > 0
    tempFig=setFigure_tj; set(gcf, 'Position', [100, 100, 1000, 500]);
...
if length(RT_across_sessions_delay) > 0 & bxData.rewardDelayPercent == 100
    ...
    ylim([200 1000]);
    savefig([summaryFigs, '\', thisMouse, '_RT_200ms_summary']);
end

if length(RT_across_sessions_delay) > 0 & bxData.rewardDelayPercent == 0
    tempFig=setFigure_tj; plot(RT_across_sessions_delay, 'b');
...
if length(RT_across_sessions_1000ms_delay) > 0
    tempFig=setFigure_tj; plot(RT_across_sessions_1000ms_delay);
    ...
    savefig([summaryFigs, '\', thisMouse, '_RT_1200ms_summary']);
end

%% plot # of licks in iti window vs pre-cue window
tempFig=setFigure_tj; errorbar(iti_lick_window_avg, iti_lick_rate_sem, 'k');
hold on; 
...
%%
%save vars in a logical and central place for easy reference
new_out_dir = '\\duhs-user-nc1.dhe.duke.edu\dusom_glickfeldlab\All_Staff\home\tj\test\Behavior\summaries';
save([new_out_dir, thisMouse(1:4), 'summary_data'], 'RT_across_days', 'RT_across_days_sem', 'RT_across_days_b2', 'RT_across_days_sem_b2',...
    'miss_rates', 'miss_rates_b2', 'TFT_rates', 'TFT_rates_b2');



% lick histogram across CS+ and CS- trials in a session
% tempFig=setFigure_tj; bar(x_axis_bin, full_trial_licks_rewarded_bin, 'black'); 
% hold on; 
% erR = errorbar(x_axis_bin,full_trial_licks_rewarded_bin,(-full_trial_licks_rewarded_bin_ste(:,:)),full_trial_licks_rewarded_bin_ste);    
% erR.Color = [0 0 0]; erR.LineStyle = 'none'; 
