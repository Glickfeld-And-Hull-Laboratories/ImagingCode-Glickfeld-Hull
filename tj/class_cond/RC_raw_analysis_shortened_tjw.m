...
fName = fullfile(bdata_source, ['data-' mouse '-' date '-' time '.mat']); %find behavior data
load(fName);
...
%% SECTION TWO - 
    ...
    if isfield(b_data, 'lickometerTimesUs')  %determine if input is structure array field
        for kk = 1:length(b_data.lickometerTimesUs) % get rid of the first trial
    ...
    %REWARDED plot licking histograms for rewarded trials
            ...
            title(['Rewarded Trials: cumulative hist of licking img', mouse, ' ' date ' n=' num2str(size(licks_by_trial_rewarded,1))]);
            hold on;
