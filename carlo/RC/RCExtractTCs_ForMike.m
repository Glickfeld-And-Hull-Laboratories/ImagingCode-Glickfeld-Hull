...
cut = [];
region = '_'; %fixed value that keeps indexing consistent, but allows modification for multi ROI imaging sessions
...
if ttl == 1
    load(['img' mouse '_000_' run '_realtime.mat'])
...
if write_tiff && ~exist([analysis_out, img_fn, date '_img' mouse '_*k.tif'])  
...
nIC =         input('nIC count: '); % cannot be bigger than nPCA. If CoEvals doesn't change in later ICs, it will not converge!
...
startRemove = input('remove any ROIs? (1-yes | 2-no): '); 
if startRemove == 1
doneRemove = 2;
while doneRemove == 2
...
hold on;
sgtitle([date '_img' mouse '_' region '_masks_final']);
savefig([analysis_out,img_fn date '_img' mouse '_' run, '_nPCA', num2str(nPCA),...
    '_mu', num2str(mu), '_nIC', num2str(nIC), '_thresh', num2str(cluster_threshold), ...
    '_coor' num2str(threshold), region 'mask_wdendrites_final.fig']);
save([analysis_out,img_fn date '_img' mouse '_' run, '_nPCA', ...
    num2str(nPCA), '_mu', num2str(mu), '_nIC', num2str(nIC), '_thresh', ...
    num2str(cluster_threshold), '_coor' num2str(threshold), region 'mask3D_final.mat'], 'mask3D');
...
savefig([analysis_out,img_fn date '_img' mouse '_' run, '_nPCA', num2str(nPCA),...
    '_mu', num2str(mu), '_nIC', num2str(nIC), '_thresh', num2str(cluster_threshold), region 'TC.fig']);
mask_flat = plotMask_Jin(mask3D,1); %second input is either 0 or 1, if 1, makes a figure.
savefig([analysis_out,img_fn date '_img' mouse '_' run, '_nPCA', num2str(nPCA),...
    '_mu', num2str(mu), '_nIC', num2str(nIC), '_thresh', num2str(cluster_threshold), region 'mask_plotMask.fig']);

data_corr = corrcoef(tc_avg);
figure; fig = imagesc(data_corr);
savefig([analysis_out,img_fn date '_img' mouse '_' run, '_nPCA', num2str(nPCA),...
    '_mu', num2str(mu), '_nIC', num2str(nIC), '_thresh', num2str(cluster_threshold), region 'corrcoef.fig']);

save([analysis_out,img_fn date '_img' mouse '_' run, '_nPCA', ...
    num2str(nPCA), '_mu', num2str(mu), '_nIC', num2str(nIC), '_thresh', ...
    num2str(cluster_threshold), region '_TCave.mat'], 'tc_avg','raw_tc_avg','pockel_tc','cutoffF');
...
writetiff(rgs_align_avg, fullfile([analysis_out,img_fn date '_img' mouse '_' run, region 'cueAlign.tif']));

save([analysis_out,img_fn date '_img' mouse '_' run, '_nPCA', ...
    num2str(nPCA), '_mu', num2str(mu), '_nIC', num2str(nIC), '_thresh', ...
    num2str(cluster_threshold), region '_TCave.mat'],'cut', 'tc_avg', 'tc_avg_all','cTargetOn_cutted','laseron','-append');

save([analysis_out,img_fn date '_img' mouse '_' run, region 'saveOutputs.mat'], 'nPCA','mu','nIC','cluster_threshold','threshold','ref');