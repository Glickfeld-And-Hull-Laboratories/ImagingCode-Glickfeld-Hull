clear;
analysis_out = 'A:\home\carlo\analysis\2P\RC\'; %output folder
bdata_source = 'A:\home\carlo\rawData\behavioral\'; %input folder
write_tiff = true; do_jake_pockel = true; 
ttl =   1;
noreg = 0;
imgreglaseron = 0;
mouse = num2str(input('mouseID: ')); %'1078'
date = num2str(input('date: ')); %'201217'
run = num2str(input('run: ')); %'000'
sessions = [date '_img' mouse]; %for behavioral data
...
img_fn2 = sessions;
mworks = getBxData_Carlo(bdata_source, img_fn2);
...
nIC =         150; % cannot be bigger than nPCA. If CoEvals doesn't change in later ICs, it will not converge! - number selected based on images with visible dendrites
