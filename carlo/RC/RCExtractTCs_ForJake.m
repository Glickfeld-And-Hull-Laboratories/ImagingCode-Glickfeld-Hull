clear;
analysis_out = 'A:\home\carlo\jakeAnalysis\deconAnalysis\';
bdata_source = 'A:\home\carlo\jakeAnalysis\rawData\behavioral\';
write_tiff = true; do_jake_pockel = true;
ttl =   1;
noreg = 0;
imgreglaseron = 0;
sessions = '190424_img096';% for behavior data
mouse = '096';
date = '190424';
run = '000';
...
cd(fullfile('A:\home\carlo\jakeAnalysis\rawData\2P\', [date '_img' mouse]));
...
if ttl == 1
    ...
    if do_jake_pockel %&& length(unique(ttl_log)) == 1 || sum(ttl_log) < 0.05*(length(ttl_log))  %if ttl_log is faulty then identify pockel transitions via changes in F
        ...
        data_cut = data;
        data_cut(:,:,(ttl_ind))=[]; %ttl_ind gives the number of each trial where the laser was off; and ttl_log (from which it is derived) provides a logical matrix for laseron/off, so this is used to index into data to pull out only the trials where the laser was on (based on changes in fluorescence)
    end
else
    if nf > 60000
        nf = 60000;
...
if exist('pockel_tc', 'var')==1
laseron = find(pockel_tc==1);
activityRef = 'pockel cell'
else
laseron = ones(length(ttl_log),1); laseron(ttl_ind,1) = 0;
activityRef = 'fluorescence change'
end
...
nIC =         input('Based on PCA output, nIC should be \n ');
...
save([analysis_out,img_fn date '_img' mouse '_' run, '_nPCA', ...
    num2str(nPCA), '_mu', num2str(mu), '_nIC', num2str(nIC), '_thresh', ...
    num2str(cluster_threshold), '_TCave.mat'], 'tc_avg','ttl_log'); %pockel_tc changed to ttl_log as that was used to decide laseron periods

%% make movie and revert tc_avg to inital counters by adding Nans

tc_avg_all = NaN(length(ttl_log),nmask);
...
save([analysis_out,img_fn date '_img' mouse '_' run, '_nPCA', ...
    num2str(nPCA), '_mu', num2str(mu), '_nIC', num2str(nIC), '_thresh', ...
    num2str(cluster_threshold), '_TCave.mat'],'cut', 'tc_avg_all','cTargetOn_cutted','laseron','activityRef','-append');
