...
bxSourceBase = 'A:\home\mike\Data\Behavior\'; %base folder for data from bx session
bxOutputBase = 'A:\home\carlo\mikeAnalysis\behavioral\'; %base folder for output for bx analysis
crpFigBase = 'A:\home\carlo\mikeAnalysis\crpFigures\'; %base folder for output for figures

timeBeforeCue = 2000; %defines the window around the cue presentation which will be taken for plotting
timeAfterCue = 3000;

CRP_training_days_lists_mike; %load blank variable arrays
...
for ii = 1:length(days)
    ...
    bxData = getBxData_ForMike(bxSourceBase, days{ii});  %find the correct behavior file and loads it.
...
saveas(gcf,[sessionFigs '\' thisDate '_img' thisMouse '_stackedLickHistSEM']);
