...
%% FOR LOOP ADDED TO COMPARE DIFFERENT RT LIMITS
...
sessionFigs = [crpFigBase, thisDate, '_img', thisMouse, '_sessionNoBurst\'];
    %name folder for xSession figures
summaryFigs = [crpFigBase, thisDate, '_img', thisMouse, '_summaryNoBurst\'];
    %check if exists, if not, make file
if exist([crpFigBase, thisDate, '_img', thisMouse, '_sessionNoBurst\'], 'file') == 0
    mkdir([crpFigBase, thisDate, '_img', thisMouse, '_sessionNoBurst\']);
end
if exist([crpFigBase, thisDate, '_img', thisMouse, '_summaryNoBurst\'], 'file') == 0
    mkdir([crpFigBase, thisDate, '_img', thisMouse, '_summaryNoBurst\']);
end
...
%         end
    ...
    all_trials_lick_hist = all_trials_lick_hist; %A=A; why does this exist
    ...
    if exist([sessionFigs, days{ii}, '_rew_cum_hist.fig'], 'file')==0
        ...
        hold on;
        for kk = 1:6:size(all_trials_lick_hist,1) %1:72 counting every 6th
    ...
    RT_this_session=zeros(bxData.trialSinceReset,25);
    if RT_this_session==0
        RT_this_session = NaN
    end
...
%{ 
 ...
 %}
limitValues = {200,1500; 100,1500; 0,1500; 700,2000; 700,1500; 700,2500; 200,1000; 200,2000; 200,3000}; 

for limitIteration = 1:length(limitValues)
    ...
    for kk = 1:numTrials-1 %look at each trial excluding the last
         ...
         if length(this_trial_post_cue_licks) >=1 %at least three licks needed to define a burst
             for i = 1:length(this_trial_post_cue_licks) %look ahead as long as three licks can still be used
                no_burst_flag = true; 
%%this section was removed to isolate individual licks                
%                 if  this_trial_post_cue_licks(i+2)-this_trial_post_cue_licks(i) <= 300 %300ms window for bursting
%                     no_burst_flag = false;
...
%                 end
            ...
            if no_burst_flag % flagged when there are many licks, but no bursts. Use first instead
               this_trial_RT = this_trial_post_cue_licks(1:i);
...
%changed from 200<RT_this_session<500 to 500<RT_this_session<1500
    ...
    if bxData.doBlock2 > 0
        ...
        if exist([sessionFigs, days{ii}, 'block2Hist.fig'], 'file')==0
            ...
            xlabel(['No reward delivered at ' num2str(bxData.RewardDelayDurationMs), 'ms']);
            ylabel('lick rate (Hz)');
            vline(bxData.RewardDelayDurationMs,'b');
