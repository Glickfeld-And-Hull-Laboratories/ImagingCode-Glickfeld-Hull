...
%{ 
 ...
 %}
    ...
    if TFT_ms > 0 && bxData.RewardDelayDurationMs == 0 && rtTrimToggle %%% what is this?
    ...
    if exist([sessionFigs, days{ii}, '_RT_plot.fig'], 'file')==0
        tempFig=setFigure; plot(RT_this_session);
    ...
    if bxData.doBlock2 ==1
        if exist([sessionFigs, days{ii}, '_RT_plot_block2.fig'], 'file')==0;
            tempFig=setFigure; plot(RT_this_session_block2);
...
%         full_trial_licks_block2_binSecondQuart = zeros(1,(length(full_trial_licks_block2_sum)/bin_size));
    ...
    if exist([sessionFigs, days{ii}, '_rew_lick_hist.fig'], 'file')==0 
        tempFig=setFigure; bar(x_axis_bin, full_trial_licks_rewarded_bin);
    ...
    if bxData.rewardOmissionPercent >0
        if exist([sessionFigs, days{ii}, '_om_lick_hist.fig'], 'file')==0
            tempFig=setFigure; bar(x_axis_bin, full_trial_licks_omit_bin);
    ...
    if bxData.rewardUnexpectPercent >0
        if exist([sessionFigs, days{ii}, '_unexp_lick_hist.fig'], 'file')==0;
            tempFig=setFigure; bar(x_axis_bin, full_trial_licks_unexp_bin);
    ...
    if bxData.doBlock2 > 0
        ...
        if exist([sessionFigs, days{ii}, '_block2_lick_hist.fig'], 'file')==0
            tempFig=setFigure; bar(x_axis_bin, full_trial_licks_block2_bin);
...
if bxData.rewardOmissionPercent > 1
    tempFig=setFigure; subplot(1,2,2); 
...
if length(days_divider_inx) > 0
    tempFig=setFigure; set(gcf, 'Position', [100, 100, 1000, 500]);
...
if length(RT_across_sessions_delay) > 0 & bxData.rewardDelayPercent == 0
    tempFig=setFigure; plot(RT_across_sessions_delay, 'b');
...
if length(RT_across_sessions_1000ms_delay) > 0
    tempFig=setFigure; plot(RT_across_sessions_1000ms_delay);
    ...
    savefig([summaryFigs, '\', thisMouse, '_RT_1500ms_summary']);
end

%% plot # of licks in iti window vs pre-cue window
tempFig=setFigure; errorbar(iti_lick_window_avg, iti_lick_rate_sem, 'k');
hold on; 
...
% lick histogram across CS+ and CS- trials in a session
% tempFig=setFigure; bar(x_axis_bin, full_trial_licks_rewarded_bin, 'black'); 
% hold on; 
% erR = errorbar(x_axis_bin,full_trial_licks_rewarded_bin,(-full_trial_licks_rewarded_bin_ste(:,:)),full_trial_licks_rewarded_bin_ste);    
% erR.Color = [0 0 0]; erR.LineStyle = 'none'; 
