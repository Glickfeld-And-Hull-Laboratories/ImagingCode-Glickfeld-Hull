function [os,osv,ds,Zp,Zc,Zpv,Zcv,ar] = lgnaggregratephgen2(sx,sy,nc,thresh,c50,PA,positions,nphases)
...
% 16 orientations, 90 degree plaids
if PA == 90
    oris = 0:22.5:337.5;
end
...
phh = 0:2*pi/nphases:(2*pi - (2*pi/nphases));
...
% For firing rate
for j=1:length(phh)
    [Rp(j), Rc(j)] = plcomptest(f1, pf1(j,:),PA);
    Zp(j) = ztrans_right([Rp(j) Rc(j)],12);
    Zc(j) = ztrans_right([Rc(j) Rp(j)],12);
end
% For Vm
for j=1:length(phh)
    [Rpv(j), Rcv(j)] = plcomptest(f1v, pf1v(j,:),PA);
