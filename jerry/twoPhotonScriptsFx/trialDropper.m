...
if isfield(inputStruct,'stimTimingSource')
    ...
    warning('No defined stim on timing source detected');
end
...
for i=1:length(fdnames)
    ...
    if length(this_fd)==nTrials && ~strcmp(fdnames(i), 'savedEvents')
        ...
        elseif action == "REP"
            ...
            error('Unspecified or incorrect action input. DEL- deletes trials REP-replaces with NaN.')
        end
