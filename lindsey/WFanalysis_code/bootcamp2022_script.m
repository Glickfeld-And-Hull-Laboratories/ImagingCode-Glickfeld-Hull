...
roi_data = [];
for i = 1:length(time)
    cd(fullfile(data_pn, [date '_' mouse], [mouse '_' run num2str(i)]));
...
nStim = input.gratingContrastStepN; %identify how many stimuli were shown
...
%% stim frames
frame_mat = reshape(1:nTotalFrames,[nOffs+nOns nTrials]);
con_mat = celleqel2mat_padded(input.tGratingContrast);
stimFrames = reshape(frame_mat(nOffs+1:end,:), [nOns*nTrials 1]);
stim_mat = zeros(1,nTotalFrames);
stim_mat(stimFrames) = 1;
stim_con_mat = reshape(reshape(stim_mat,[nOffs+nOns nTrials]).*con_mat,[(nOffs+nOns)*nTrials 1]);
figure; plot(stim_con_mat)
...
area_list = strvcat('LM','V1','AL','R1','R2','PM','AM');
% ind = find(vasc_mask); 
...
roiTCnpsub_DFoverF_avg = squeeze(mean(roiTCnpsub_DFoverF_stim,3));
roiTCnpsub_DFoverF_sem = squeeze(std(roiTCnpsub_DFoverF_stim,[],3)./sqrt(double(nRep)));
col_mat = {'b','g','y'};
figure;
[n n2] = subplotn(nROI);
for i = 1:nROI
    subplot(n,n2,i)
    for ii = 1:nStim
        shadedErrorBar(tt, roiTCnpsub_DFoverF_avg(:,i,ii), roiTCnpsub_DFoverF_sem(:,i,ii),'lineprops',col_mat{ii});
        hold on
    end
    vline(0)
    title(area_list(i,:))
...
[wheel_speed] = wheelSpeedCalc(input,32,'purple'); %add purple wheel!
...
RIx = abs(wheel_trial_avg)>0.2;
ind1 = find(wheel_trial_avg<=0);
ind2 = find(wheel_trial_avg>0 & wheel_trial_avg<0.2);
ind3 = find(wheel_trial_avg>=0.2);

%% correlation with running vs stimulus
roiTC_DFoverF_long = reshape(permute(reshape(roiTC_DFoverF_stim,[(nOns+nOffs) nROI nTrials]),[1 3 2]),[(nOns+nOffs).*nTrials nROI])';
roiTC_long_smooth = zeros(size(roiTC_DFoverF_long));
for i = 1:nROI
    roiTC_long_smooth(i,:) = smooth(roiTC_DFoverF_long(i,:),10);
end
wheel_smooth = smooth(wheel_speed,10);
figure;
subplot(2,1,1)
tcOffsetPlot(roiTC_long_smooth')
xlabel('Frames')
ylabel('dF/F')
subplot(4,1,3)
plot(wheel_smooth)
...
subplot(4,1,4)
plot(stim_con_mat)
ylabel('Stimulus times')

r_wheel = zeros(nROI,1);
r_stim = zeros(nROI,1);
r_con = zeros(nROI,1);
for i = 1:nROI
    r_wheel(i) = triu2vec(corrcoef(wheel_smooth,roiTC_long_smooth(i,:)));
    r_con(i) = triu2vec(corrcoef(stim_con_mat,roiTC_long_smooth(i,:)));
    r_stim(i) = triu2vec(corrcoef(stim_mat,roiTC_long_smooth(i,:)));
end
figure;
subplot(1,2,1)
for i = 1:nROI
    plot(r_wheel(i,:),r_stim(i,:),'o')
    hold on
end
xlim([-.3 .3])
ylim([-.3 .3])
xlabel('Wheel correlation')
ylabel('Stimulus correlation')
refline(1)
vline(0)
hline(0)
legend(area_list, 'Location', 'southeast')
subplot(1,2,2)
plot(tt, mean(roiTCnpsub_DFoverF_avg,3))
ylim([-.01 0.05])
ylabel('dF/F')
hold on
vline(0)
...
subplot(1,2,1)
for i = 1:nROI
    plot(r_stim(i,:),r_con(i,:),'o')
    hold on
end
xlim([0 .3])
ylim([0 .3])
xlabel('Stimulus correlation')
ylabel('Contrast correlation')
refline(1)
vline(0)
hline(0)
