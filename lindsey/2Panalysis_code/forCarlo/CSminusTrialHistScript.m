...
%load mworks data
fName = ['\\duhs-user-nc1.dhe.duke.edu\dusom_glickfeldlab\All_staff\home\carlo\rawData\behavioral\data-i' mouse '-' date '-' time '.mat'];
load(fName);
b2Ix = celleqel2mat_padded(input.tBlock2TrialNumber);
%load events tc
fData = ['\\duhs-user-nc1.dhe.duke.edu\dusom_glickfeldlab\All_staff\home\carlo\analysis\2P\' date '_img' mouse '\getTC_001\figDataUntrimmed_' date '_img' mouse '.mat'];
load(fData);
