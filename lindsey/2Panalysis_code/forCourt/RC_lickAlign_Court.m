...
run = expt(exp).run{session};
    ...
    %load behavior file
    load(bfile);
    load(fullfile(analysis_out,img_fn,[date '_img' mouse '_targetAlign.mat']));
    ...
    vline(0,'k');
    vline(700,'r');
    savefig(fullfile(analysis_out,img_fn,[date '_img' mouse  '_cueAlign_lickHz.fig']));
    print(fullfile(analysis_out,img_fn,[date '_img' mouse  '_cueAlign_lickHz.pdf']),'-dpdf','-bestfit')

    figure; %auditory/visual
    subplot(2,1,1)
    shadedErrorBar(tt, nanmean(lickCueAlign(:,intersect(find(tVis==0),find(tRew==1))),2).*(1000./frameRateHz), (nanstd(lickCueAlign(:,intersect(find(tVis==0),find(tRew==1))),[],2)./sqrt(unique(sum(~isnan(lickCueAlign(:,intersect(find(tVis==0),find(tRew==1)))),2))).*(1000./frameRateHz)));
    hold on;
    shadedErrorBar(tt, nanmean(lickCueAlign(:,intersect(find(tVis==1),find(tRew==1))),2).*(1000./frameRateHz), (nanstd(lickCueAlign(:,intersect(find(tVis==1),find(tRew==1))),[],2)./sqrt(unique(sum(~isnan(lickCueAlign(:,intersect(find(tVis==1),find(tRew==1)))),2))).*(1000./frameRateHz)),'lineProps', {'b','markerfacecolor','b'});
    xlabel('Time from cue');
    ylabel('Lick rate (Hz)');
    title('Rewarded')
    vline(0,'k');
    ...
    subplot(2,1,2)
    shadedErrorBar(tt, nanmean(lickCueAlign(:,intersect(find(tVis==0),find(tRew==0))),2).*(1000./frameRateHz), (nanstd(lickCueAlign(:,intersect(find(tVis==0),find(tRew==0))),[],2)./sqrt(unique(sum(~isnan(lickCueAlign(:,intersect(find(tVis==0),find(tRew==0)))),2))).*(1000./frameRateHz)));
    hold on;
    shadedErrorBar(tt, nanmean(lickCueAlign(:,intersect(find(tVis==1),find(tRew==0))),2).*(1000./frameRateHz), (nanstd(lickCueAlign(:,intersect(find(tVis==1),find(tRew==0))),[],2)./sqrt(unique(sum(~isnan(lickCueAlign(:,intersect(find(tVis==1),find(tRew==0)))),2))).*(1000./frameRateHz)),'lineProps', {'b','markerfacecolor','b'});
    xlabel('Time from cue');
    ylabel('Lick rate (Hz)');
    title('Unrewarded')
    sgtitle([date ' ' mouse '' run]);
    vline(0,'k');
    vline(700,'r');
    
    savefig(fullfile(analysis_out,img_fn,[date '_img' mouse  '_cueAlign_lickHz_VisAud.fig']));
    print(fullfile(analysis_out,img_fn,[date '_img' mouse  '_cueAlign_lickHz_VisAud.pdf']),'-dpdf','-bestfit')
    ...
    if sum(~isnan(lickBurstStart))>6
        ...
        shadedErrorBar(tt,nanmean(nanmean(targetAlign_events(:,:,ind_early_bst),3),2).*(1000./frameRateHz), (nanstd(nanmean(targetAlign_events(:,:,ind_early_bst),3),[],2).*(1000./frameRateHz))./sqrt(nIC));
        hold on;
        shadedErrorBar(tt,nanmean(nanmean(targetAlign_events(:,:,ind_late_bst),3),2).*(1000./frameRateHz), (nanstd(nanmean(targetAlign_events(:,:,ind_late_bst),3),[],2).*(1000./frameRateHz))./sqrt(nIC),'lineProps', {'b','markerfacecolor','b'});
        ...
        savefig(fullfile(analysis_out,img_fn, [date '_img' mouse '_cueAlignSpiking_byLickTime.fig']));
        hold off;
    end
    ...
    shadedErrorBar(tl, nanmean(nanmean(postRew_lickAlignEvents,3),2).*(1000./frameRateHz), (nanstd(nanmean(postRew_lickAlignEvents,3),[],2).*(1000./frameRateHz))./sqrt(nIC));
    hold on;
    ...
    shadedErrorBar(tl, nanmean(nanmean(postRew_lickAlignEvents(:,:,ind_prerew_early_bst),3),2).*(1000./frameRateHz), (nanstd(nanmean(postRew_lickAlignEvents(:,:,ind_prerew_early_bst),3),[],2).*(1000./frameRateHz))./sqrt(nIC));
    hold on;
    shadedErrorBar(tl, nanmean(nanmean(postRew_lickAlignEvents(:,:,ind_prerew_late_bst),3),2).*(1000./frameRateHz), (nanstd(nanmean(postRew_lickAlignEvents(:,:,ind_prerew_late_bst),3),[],2).*(1000./frameRateHz))./sqrt(nIC),'lineProps', {'b','markerfacecolor','b'});
    xlabel('Time from lick');
    ylabel('Spike rate (Hz)');
    ylim([0 inf]);
    ...
    savefig(fullfile(analysis_out,img_fn, [date '_img' mouse  '_postRew_lickBurstAlignSpikingEvents.fig']))

    figure;
    subplot(2,1,1); % align licking data to first lick
    shadedErrorBar(tl, nanmean(postRew_lickAlign,2).*(1000./frameRateHz), (nanstd(postRew_lickAlign,[],2).*(1000./frameRateHz))./sqrt(unique(sum(~isnan(postRew_lickAlign),2))));
    hold on;
    xlabel('Time from lick');
    ylabel('Lick rate (Hz)');
    ylim([0 inf]);
    ...
    shadedErrorBar(tl, nanmean(postRew_lickAlign(:,ind_prerew_early_bst),2).*(1000./frameRateHz), (nanstd(postRew_lickAlign(:,ind_prerew_early_bst),[],2).*(1000./frameRateHz))./sqrt(length(ind_prerew_early_bst)));
    hold on;
    shadedErrorBar(tl, nanmean(postRew_lickAlign(:,ind_prerew_late_bst),2).*(1000./frameRateHz), (nanstd(postRew_lickAlign(:,ind_prerew_late_bst),[],2).*(1000./frameRateHz))./sqrt(length(ind_prerew_late_bst)),'lineProps', {'b','markerfacecolor','b'});
    xlabel('Time from lick');
    ylabel('Lick rate (Hz)');
    ylim([0 inf]);
    title('Early burst trials [blk] and late burst trials [blu]')
    supertitle([mouse ' ' date '- post reward lick burst aligned spiking']);
    savefig(fullfile(analysis_out,img_fn, [date '_img' mouse  '_postRew_lickBurstAlignSpiking.fig']));
    hold off;
    ...
    hold off;
    savefig(fullfile(analysis_out,img_fn,[date '_img' mouse '_cumulativeLicking.fig']));
    print(fullfile(analysis_out,img_fn,[date '_img' mouse '_cumulativeLicking.pdf']),'-dpdf','-bestfit');
    ...
    subplot(2,2,1);
    shadedErrorBar(tl_rew, nanmean(nanmean(lastPreRew_lickAlignEvents,3),2).*(1000./frameRateHz), (nanstd(nanmean(lastPreRew_lickAlignEvents,3),[],2).*(1000./frameRateHz))./sqrt(nIC));
    hold on;
    ...
    ylim([0 inf]);
    subplot(2,2,3);
    shadedErrorBar(tl_rew, nanmean(lastPreRew_lickAlign,2).*(1000./frameRateHz), (nanstd(lastPreRew_lickAlign,[],2).*(1000./frameRateHz))./sqrt(unique(sum(~isnan(lastPreRew_lickAlign)))));
    xlabel('Time from lick (ms)');
    ylabel('Lick rate (Hz)');
    ylim([0 30]);
    title(['Pre-rew: ' num2str(length(preRewTrials))]);
    subplot(2,2,2);
    shadedErrorBar(tl_rew, nanmean(nanmean(firstPostRew_lickAlignEvents,3),2).*(1000./frameRateHz), (nanstd(nanmean(firstPostRew_lickAlignEvents,3),[],2).*(1000./frameRateHz))./sqrt(nIC));
    title('First lick after reward');
    xlabel('Time from lick (ms)');
    ylabel('Spike rate (Hz)');
    ylim([0 inf]);
    subplot(2,2,4);
    shadedErrorBar(tl_rew, nanmean(firstPostRew_lickAlign,2).*(1000./frameRateHz), (nanstd(firstPostRew_lickAlign,[],2).*(1000./frameRateHz))./sqrt(unique(sum(~isnan(firstPostRew_lickAlign)))));
    xlabel('Time from lick (ms)');
    ylabel('Lick rate (Hz)');
    ylim([0 inf]);
    ...
    hold off;
    savefig(fullfile(analysis_out,img_fn, [date '_img' mouse '_lastVsFirstLick.fig']));
    ...
    subplot(3,1,1);
    shadedErrorBar(tt,nanmean(nanmean(targetAlign_events(:,:,ind_low_bst),3),2).*(1000./frameRateHz), (nanstd(nanmean(targetAlign_events(:,:,ind_low_bst),3),[],2).*(1000./frameRateHz))./sqrt(nIC),'lineProps', {'b','markerfacecolor','b'});
    hold on;
    shadedErrorBar(tt,nanmean(nanmean(targetAlign_events(:,:,ind_high_bst),3),2).*(1000./frameRateHz), (nanstd(nanmean(targetAlign_events(:,:,ind_high_bst),3),[],2).*(1000./frameRateHz))./sqrt(nIC));
    xlabel('Time from cue');
    ylabel('Spike rate (Hz)');
    ylim([-1 inf]);
    title(['1s- Rew: ' num2str(chop(HL_lickrate.low_rew,2)) ' vs ' num2str(chop(HL_lickrate.high_rew,2)) ' Hz']);
    subplot(3,1,2);
    shadedErrorBar(tt,nanmean(nanmean(targetAlign_events(:,:,ind_low_prerew),3),2).*(1000./frameRateHz), (nanstd(nanmean(targetAlign_events(:,:,ind_low_prerew),3),[],2).*(1000./frameRateHz))./sqrt(nIC),'lineProps', {'b','markerfacecolor','b'});hold on;
    shadedErrorBar(tt,nanmean(nanmean(targetAlign_events(:,:,ind_high_prerew),3),2).*(1000./frameRateHz), (nanstd(nanmean(targetAlign_events(:,:,ind_high_prerew),3),[],2).*(1000./frameRateHz))./sqrt(nIC));
    xlabel('Time from cue');
    ylabel('Spike rate (Hz)');
    ylim([-1 inf]);
    title(['Pre- Rew: ' num2str(chop(HL_lickrate.low_prerew,2)) ' vs ' num2str(chop(HL_lickrate.high_prerew,2)) ' Hz']);
    subplot(3,1,3);
    shadedErrorBar(tt,nanmean(nanmean(targetAlign_events(:,:,ind_low_postrew),3),2).*(1000./frameRateHz), (nanstd(nanmean(targetAlign_events(:,:,ind_low_postrew),3),[],2).*(1000./frameRateHz))./sqrt(nIC),'lineProps', {'b','markerfacecolor','b'});hold on;
    shadedErrorBar(tt,nanmean(nanmean(targetAlign_events(:,:,ind_high_postrew),3),2).*(1000./frameRateHz), (nanstd(nanmean(targetAlign_events(:,:,ind_high_postrew),3),[],2).*(1000./frameRateHz))./sqrt(nIC));
    xlabel('Time from cue');
    ylabel('Spike rate (Hz)');
    ylim([-1 inf]);
    ...
    hold off;
    savefig(fullfile(analysis_out,img_fn, [date '_img' mouse '_cueAlignSpiking_byLickRate.fig']));
    
    save(fullfile(analysis_out,img_fn, [date '_img' mouse '_cueAlignLick.mat']), 'firstPostRewLickFrame', ...
