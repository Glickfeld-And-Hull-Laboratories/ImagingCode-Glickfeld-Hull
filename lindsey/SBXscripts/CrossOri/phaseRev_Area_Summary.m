...
for iarea = 1:narea
    ...
    phaseRev(iarea).threshCells = length(find(f1_all>0.02));
    ...
    hold on
    subplot(3,2,3)
    errorbar(iarea, mean(f2overf1_all(find(f1_all>0.02))),std(f2overf1_all(find(f1_all>0.02)))./sqrt(phaseRev(iarea).threshCells),'o');
    hold on
    subplot(3,2,5)
    errorbar(mean(f1_all(find(f1_all>0.02))),mean(f2_all(find(f1_all>0.02))),std(f1_all(find(f1_all>0.02)))./sqrt(phaseRev(iarea).threshCells),std(f1_all(find(f1_all>0.02)))./sqrt(phaseRev(iarea).threshCells),std(f2_all(find(f1_all>0.02)))./sqrt(phaseRev(iarea).threshCells),std(f2_all(find(f1_all>0.02)))./sqrt(phaseRev(iarea).threshCells),'o');
    hold on
...
title('30 Hz')
subplot(3,2,4)
errorbar(1, mean(f2overf1_all(find(f1_all>0.02))),std(f2overf1_all(find(f1_all>0.02)))./sqrt(phaseRev.threshCells),'o');
hold on
xlim([0 2])
ylim([0 1])
...
ylabel('F2/F1')
subplot(3,2,6)
errorbar(mean(f1_all(find(f1_all>0.02))),mean(f2_all(find(f1_all>0.02))),std(f1_all(find(f1_all>0.02)))./sqrt(phaseRev.threshCells),std(f1_all(find(f1_all>0.02)))./sqrt(phaseRev.threshCells),std(f2_all(find(f1_all>0.02)))./sqrt(phaseRev.threshCells),std(f2_all(find(f1_all>0.02)))./sqrt(phaseRev.threshCells),'o');
hold on
xlabel('F1')
ylabel('F2')
xlim([0 0.15])
...
print(fullfile(summaryDir,['phaseRev_Area_Summary.pdf']),'-dpdf','-bestfit')
