...
for iexp = 1:nexp
    ...
    amp_all = [amp_all; amp_hat];
    amp_shuf_all = [amp_shuf_all; amp_hat_shuf];
    yfit_allcells = [yfit_allcells; yfit];
    yfit_shuf_allcells = [yfit_shuf_allcells; yfit_shuf];
    anova_all = [anova_all; p_anova];
    pha_all = [pha_all; pha_hat];
    b_all = [b_all; b_hat];
end

tt= (-prewin_frames:postwin_frames-1).*(1000./frame_rate);
save(fullfile(manuscriptDir, 'FigureS5_CalciumImaging_data.mat'),'anova_all','resp_ind_all_phase','amp_all','pha_all',...
'amp_shuf_all', 'b_all', 'phase_SI_all', 'phase_MI_all', 'phase_MI_max_all', 'mouse_list','expt','expUse','resp_tc_all','tt','expt_ind',...
'SI_avg_all','yfit_allcells','maskPhas','nMaskPhas')