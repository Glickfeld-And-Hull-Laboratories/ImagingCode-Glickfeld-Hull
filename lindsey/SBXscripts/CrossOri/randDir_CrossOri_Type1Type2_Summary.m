...
for iTF = 1:nTF
    subplot(2,2,iTF)
    scatter(Zc_all(intersect(find(DSI_all(:,iTF,1,1)>0.5),resp_ind_all{iTF}),iTF), Zp_all(intersect(find(DSI_all(:,iTF,1,1)>0.5),resp_ind_all{iTF}),iTF))
    hold on
    scatter(Zc_all(intersect(find(TF_pref_all==iTF),intersect(find(DSI_all(:,iTF,1,1)>0.5),resp_ind_all{iTF})),iTF), Zp_all(intersect(find(TF_pref_all==iTF),intersect(find(DSI_all(:,iTF,1,1)>0.5),resp_ind_all{iTF})),iTF))
...
for iTF = 1:nTF
    ...
    indUse = intersect(intersect(find(DSI_all(:,iTF,1,1)>0.5),find(DSI_all(:,iTF,TF2,2)>0.5)),resp_ind_all{iTF});
    prefDiff = u1_dir_all(:,iTF,1,1)-u1_dir_all(:,iTF,TF2,2);
...
prefDiff = u1_dir_all(:,iTF,1,1)-u1_dir_all(:,iTF,TF2,2);
figure
indUse = intersect(intersect(intersect(find(DSI_all(:,iTF,1,1)>0.5),find(DSI_all(:,iTF,TF2,2)>0.5)),resp_ind_all{iTF}),find(Zp_all(:,iTF)-Zc_all(:,iTF)>-1.25));
subplot(2,2,1)
