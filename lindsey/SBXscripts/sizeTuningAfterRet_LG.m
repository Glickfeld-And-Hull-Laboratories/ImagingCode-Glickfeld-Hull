...
fnOut =fullfile('\\duhs-user-nc1.dhe.duke.edu\dusom_glickfeldlab\All_staff\home\lindsey\Analysis\2P',datemouse,datemouserun);

ref_run_str = ['runs-' ref];
datemouserefrun = [date '_' mouse '_' ref_run_str];
fnRef = fullfile('\\duhs-user-nc1.dhe.duke.edu\dusom_glickfeldlab\All_staff\home\lindsey\Analysis\2P',datemouse,datemouserefrun);
...
elseif doFromRef
    ...
    load(fullfile(fnRef, [date '_' mouse '_' ref_str '_reg_shifts.mat']))
    load(fullfile(fnRef, [date '_' mouse '_' ref_str '_mask_cell.mat']))
...
print(fullfile(fnOut, [date '_' mouse '_' run_str '_FOV_avg.pdf']))
...
elseif input.doSizeStim
    ...
    % print to pdf
    print(fullfile(fnOut, [date '_' mouse '_' run_str '_FOV_resp_Size.pdf']), '-dpdf')
...
save(fullfile(fnOut, [date '_' mouse '_' run_str '_stimActFOV.mat']), 'data_dfof_max', 'data_dfof_avg')


%% load cell masks from retinotopy runs

%RetImgFolder = char('001','002','003');
fprintf(['Loading masks from retinotopy runs: ' ref_run_str '\n'])

% loads 'mask_cell', 'mask_np'
load(fullfile(fnRef, [date '_' mouse '_' ref_run_str '_mask_cell.mat']))
...
% fprintf(['Loading fits from retinotopy runs: ' ref_run_str '\n'])
% fn_out = fullfile('\\CRASH.dhe.duke.edu\data\home\celine\Analysis\2p_analysis', [date '_' mouse], [date '_' mouse '_' ref_run_str], [date '_' mouse '_' ref_run_str '_lbub_fits.mat']);
% load(fn_out);
...
save(fullfile(fnOut, [date '_' mouse '_' run_str '_TCs.mat']), 'data_tc', 'np_tc', 'npSub_tc','tc_dfof')
save(fullfile(fnOut, [date '_' mouse '_' run_str '_input.mat']), 'input')

%% find centered cells
load(fullfile(fnRef, [date '_' mouse '_' ref_run_str '_lbub_fits.mat']));
...
fprintf(['Loading fits from retinotopy runs: ' ref_run_str '\n'])
fn_out = fullfile(fnRef, [date '_' mouse '_' ref_run_str '_lbub_fits.mat']);
load(fn_out);
...
save(fullfile(fnOut, [date '_' mouse '_' run_str '_centerCells.mat']),'centerCells','cellDists','goodfit_ind')
...
for iCell = 1:nCells
    ...
    if start >36
        set(gcf, 'Position', [0 0 800 1000]);
        print(fullfile(fnOut, [date '_' mouse '_' run_str '_TCs' num2str(f) '.pdf']), '-dpdf')
        start = 1;
        f= f+1;
        figure;
    end
    ...
    for iStim = 1:nSzs
        ...
        plot(squeeze(mean(tc_dfof(:,iCell,ind),3)))
        hold on
        size_tuning_mat(iStim,1,iCell) = mean(mean(tc_dfof(nOff+1:end,iCell,ind),3),1);
        size_tuning_mat(iStim,2,iCell) = std(mean(tc_dfof(nOff+1:end,iCell,ind),1),[],3)./sqrt(length(ind));
...
save(fullfile(fnOut, [date '_' mouse '_' run_str '_Tuning.mat']), 'tc_dfof', 'size_tuning_mat')
...
for i = 1:nCenterCells
    ...
    if start >16
        set(gcf, 'Position', [0 0 800 1000]);
        print(fullfile(fnOut, [date '_' mouse '_' run_str '_Tuning' num2str(f) '.pdf']), '-dpdf')
        start = 1;
        f= f+1;
        figure;
    end
...
for i = 1:nCenterCells
    ...
    if start >16
        set(gcf, 'Position', [0 0 800 1000]);
        print(fullfile(fnOut, [date '_' mouse '_' run_str '_Tuning_in' num2str(cutOffRadius) 'deg' num2str(f) '.pdf']), '-dpdf')
        start = 1;
        f= f+1;
        figure;
