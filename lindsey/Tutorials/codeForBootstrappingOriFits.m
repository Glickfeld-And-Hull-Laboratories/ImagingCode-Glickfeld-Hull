...
nboot = 1000;
y_fit = zeros(180,nCells,nboot+1);
delta_resp = zeros(nCells,nDelta,nboot);
for iC = 1:nCells
    fprintf('%d\n', iC)
    ...
    for iboot = 1:nboot+1
        for idelta = 1:nDelta
            ...
            if iboot == 1
                delta_resp(iC,idelta,1) =  nanmean(resp_dfof(iC,ind),2);
            end
            if iboot>1
            ...
            delta_resp(iC,idelta,iboot) = nanmean(resp_dfof(iC,ind_use),2);
        end
    end
    %first fit tuning curve for average of all trials
    data = squeeze(delta_resp(iC,:,1));
    [b_hat(iC,1), k_hat(iC,1), R_hat(iC,1),u_hat(iC,1),sse(iC,1),R_square(iC,1)] = miaovonmisesfit_ori(deg2rad(deltas),data);
    y_fit(:,iC,1) = b_hat(iC,1)+R_hat(iC,1).*exp(k_hat(iC,1).*(cos(2.*(deg2rad(0:1:179)-u_hat(iC,1)))-1));
    ...
    for iboot = 2:nboot+1
        ...
        [b_hat, k_hat_boot, R_hat,u_hat,sse,R_square(iC,iboot)] = miaovonmisesfit_ori(deg2rad(deltas),data);
        y_fit(:,iC,iboot) = b_hat+R_hat.*exp(k_hat_boot.*(cos(2.*(deg2rad(0:1:179)-u_hat))-1));
