function [psth, bins,nTrials] = get_psth(spike_times, trial_begin_times, varargin)
%
...
p.addParamValue('smoothWin',0,@isnumeric);
...
trial_begin_times = trial_begin_times + params.start;
...
for i = params.first_trial:length(trial_begin_times)
    ...
    ith_trial_times = ith_shifted_times(ith_shifted_times >= 0 & ith_shifted_times <= params.stop);
    % bin the spike times
...
if params.smoothWin > 0 
    psth = smoothdata(psth,2,'gaussian',params.smoothWin);
end
...
nTrials = numel(trial_begin_times);