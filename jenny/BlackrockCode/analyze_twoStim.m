...
% save_path = 'Z:\All_staff\home\jen\Analysis\twoStim_eOPN\';
save_path = 'Z:\home\jen\Analysis\twoStim_eOPN\';
...
if input.trialSinceReset > size(temp_stimOn,1)
    disp('found more stimulus info than stimulus on (typical) - cropping input file...')
    [crop_dio{count},crop_input(count)] = match_dio_stim(temp_stimOn,input,keep_fields,'input_trials',1:size(temp_stimOn,1));
elseif input.trialSinceReset < size(temp_stimOn,1)
    disp('found more stimulus on than stimulus info - cropping dio on...')
    [crop_dio{count},crop_input(count)] = match_dio_stim(temp_stimOn,input,keep_fields,'dio_trials',1:input.trialSinceReset);    
elseif size(temp_stimOn,1) == size(temp_stimOn,1)
    disp('found same number of stimulus on and stimulus info - no cropping')
    [crop_dio{count},crop_input(count)] = match_dio_stim(temp_stimOn,input,keep_fields);  
end
...
raster_spontHz = cellfun(@(x) get_raster_JL(x,(stimOn(:,1)/data.info.sampleFreq)-0.5,'stop',0.5,'count',true),spikes.spiketime_S,'un',0);
% 
data.spontHz_all = cell2mat(raster_spontHz)/0.5;
% 

% % for stim evoked activity 
raster_visHz = cellfun(@(x) get_raster_JL(x,(stimOn(:,1)/data.info.sampleFreq)+0.03,'stop',0.15,'count',true),spikes.spiketime_S,'un',0);
% 
data.visHz_all = cell2mat(raster_visHz)/0.15;
...
clear temp_ttest
raster_visStim = cellfun(@(x) get_raster_JL(x,(stimOn(:,1)/data.info.sampleFreq),'start',-0.1,'stop',0.1),spikes.spiketime_S,'un',0);
...
for stim_i = [1 2]
    [temp_psth,~] = cellfun(@(x) get_psth(x,(stimOn(:,stim_i)/data.info.sampleFreq),'start',-data.info.preStim_timeS,'bin_size',data.info.bin_sizeS,'stop',data.info.postStim_timeS,'smoothWin',gaussWin),spikes.spiketime_S,'un',0);
