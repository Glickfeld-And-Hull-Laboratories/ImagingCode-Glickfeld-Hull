...
% Loads, registers, and analyses retinotopy data for a single imaging session.
%
% Inputs:
%   day_id            - session index into expt struct
%   expt              - experiment struct (requires ret_run, ret_time fields)
%   mouse             - mouse ID string
%   inputStructure    - MWorks input struct from the main grating experiment
%   referenceFOV      - 2-D mean FOV image to register retino data onto
%   mask_cell         - 2-D labeled cell mask from step1 segmentation
%   timingSource      - 'PD', 'MW', or 'cS'
%   rc                - behaviour constants struct (rc.data path)
%   validation_choice - true/false: plot trial-averaged traces for 5 random cells
...
load(fullfile(retDataPath, [ImgFolder '_000_000.mat']));   % loads info
...
% --- Timecourses and neuropil subtraction ---
...
ret_mean_resp = squeeze(mean(ret_dfof_trial(stimFrames,:,:), 1));  % nCells x nTrials

% --- Response map by azimuth x elevation ---
...
% --- Distance from RF centre to grating centre ---
...
ret_distance = sqrt((maxAzimDeg - finalAzim).^2 + (maxElevDeg - finalElev).^2);
