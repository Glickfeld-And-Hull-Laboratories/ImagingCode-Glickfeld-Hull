...
expt(1).img_loc  = {}; %usually in the format {'region';'layer'}
expt(1).obj = '16x'; % <-- change if using the 25X objective
expt(1).zoom = ;% <--  enter the magnification 
expt(1).frame_rate = ; % <-- enter the 2p imaging framerate
...
expt(1).indicator = {''}; % <-- enter GCaMP version used
expt(1).drug = ''; % <-- enter [DART] drug used, or modify this to reflect other relevant manipulations
expt(1).greenChannelLabel = ''; % <-- if using a cell-type specific GCaMP, list the cell type here. Otherwise this is typically "any"
