...
prompt = 'Match forward (f) or reverse (r)? ';
matchDrxn = input(prompt, 's');
if matchDrxn == 'r'
    day_id(1) = str2double(instructions.session);
    if day_id(1) > length(expt)
        error('Day_id %d not valid for this dataset', day_id(1));
    end
    day_id(2) = expt(day_id(1)).multiday_matchdays;
elseif matchDrxn == 'f'
    day_id(2) = str2double(instructions.session);
    if day_id(2) > length(expt)
        error('Day_id %d not valid for this dataset', day_id(2));
    end
    day_id(1) = expt(day_id(2)).multiday_matchdays;
else
    fprintf('Please try again and enter f or r\n')
end
...
save(fullfile(fn_multi,'multiday_alignment.mat'),'cellImageAlign','fitGeoTAf', 'input_points','base_points', 'fov_avg', 'fov_norm','fov_red','dfmax','corrmap','masks','maskNP');
