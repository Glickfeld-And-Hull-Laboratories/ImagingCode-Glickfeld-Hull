...
cd(CD);
imgMatFile = [ImgFolder '000_000.mat'];
load(imgMatFile);
%Load 2P images
...
data = sbxread([ImgFolder '000_000'],0,totframes); %loads the .sbx files with imaging data (path, nframes to skip, nframes to load)
...
%writetiff(data_align_avg,fullfile(fnout,'avgMovie.tif'))
...
title([mouse ' ' date])
print(fullfile(fnout,'runAlignFOV_filtered.pdf'), '-dpdf')
save(fullfile(fnout,'stimActFOV.mat'),'data_dfof_avg_all','ind_use')
...
print(fullfile(fnout,  'masks.pdf'), '-dpdf')

save(fullfile(fnout,'mask_cell.mat'), 'mask_data', 'mask_cell', 'thresh')
...
save(fullfile(fnout, 'TCs.mat'), 'data_tc')
...
ylabel('dF/F')
title([mouse ' ' date])
print(fullfile(fnout,'avgTC_respCells.pdf'), '-dpdf')
