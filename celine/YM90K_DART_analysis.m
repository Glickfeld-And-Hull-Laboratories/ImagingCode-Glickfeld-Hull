...
for iCon = 1:nCon
    ...
    for id = 1:nd
        ...
        green_dir_avrg_loc{id}=nanmean(norm_dir_resp_loc_concat{id}(green_all,:),1);
        green_std=nanstd(norm_dir_resp_loc_concat{id}(green_all,:),[],1);
        ...
        red_dir_avrg_loc{id}=nanmean(norm_dir_resp_loc_concat{id}(red_all,:),1);
        red_std=nanstd(norm_dir_resp_loc_concat{id}(red_all,:),[],1);
...
%% making normDiff output csv - change this to include raw difference as another output
mouseID=[];
for imouse =1:nSess
