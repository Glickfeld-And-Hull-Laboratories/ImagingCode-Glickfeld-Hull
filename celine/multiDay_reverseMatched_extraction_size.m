...
for iOri = 1:nCells
red_fluor_np(iOri) = stackGetTimeCourses(redChImg, mask_np(:,:,iOri));
end
...
for id = 1:nd
    ...
    tOri_match{id} = celleqel2mat_padded(input(id).tGratingDirectionDeg(1:nTrials(id)));
...
pref_size_match{id} = cell(1,nd);
data_ori_resp_match = cell(1,nd);
data_size_resp_match = cell(1,nd);

%rect_dfof_trial=data_dfof_trial_match;
%rect_dfof_trial(find(rect_dfof_trial<0))=0;
% for each day
for id = 1:nd
    data_resp = zeros(nCells, nSize, nOri,2);
    h = zeros(nCells, nSize, nOri);
    p = zeros(nCells, nSize, nOri);
    passThresh=zeros(nCells, nSize, nOri);
    ...
    for iSize = 1:nSize
        ...
        for iOri = 1:nOri
            ...
            ind = intersect(ind_size,ind_ori); 
            data_resp(:,iSize,iOri,1) = squeeze(nanmean(nanmean(data_dfof_trial(resp_win,ind,:),1),2));
            data_resp(:,iSize,iOri,2) = squeeze(std(nanmean(data_dfof_trial(resp_win,ind,:),1),[],2)./sqrt(length(ind)));
            [h(:,iSize,iOri), p(:,iSize,iOri)] = ttest(nanmean(data_dfof_trial(resp_win,ind,:),1), nanmean(data_dfof_trial(base_win,ind,:),1),'dim',2,'tail','right','alpha',0.01./(nSize*nOri-1));
    ...
    data_size_resp = zeros(nCells,nSize); %at pref ori
    data_orth_resp=zeros(nCells,1);
    for iCell = 1:nCells
          [max_val, pref_ori(1,iCell)] = max(max(data_resp(iCell,:,:,1),[],2));
          [max_val_size, pref_size(1,iCell)] = max(max(data_resp(iCell,:,:,1),[],3)); 
          data_ori_resp(iCell,:)=data_resp(iCell,pref_size(iCell),:,1);
          data_size_resp(iCell,:)=data_resp(iCell,:,pref_ori(iCell),1);
...
data_size_resp_match{id} = data_size_resp;
% data_orth_resp_match{id}=data_orth_resp;
%  
end
clear data_resp p h_all resp pref_ori pref_size data_ori_resp data_con_resp data_dfof_trial tOri tSize data_orth_resp  baseStd baseMean thresh pass
...
%% looping through sizes and plotting mean response (averaged over contrast) across days for each cell
% all matched cells

for iSize = 1:nSize
    figure;
    scatter((data_size_resp_match{pre}(green_ind_match_list,iSize)),(data_size_resp_match{post}(green_ind_match_list,iSize)));
    hold on
    scatter((data_size_resp_match{pre}(red_ind_match_list,iSize)),(data_size_resp_match{post}(red_ind_match_list,iSize)));
    axMax =  max(max(mean(data_size_resp_match{pre}(:,iSize,:,1),3),mean(data_size_resp_match{post}(:,iSize,:,1),3)));
    axMin =  min(min(mean(data_size_resp_match{pre}(:,iSize,:,1),3),mean(data_size_resp_match{post}(:,iSize,:,1),3)));
    xlim([axMin axMax]);
    ylim([axMin axMax]);
    xlabel('Pre');
    ylabel('Post');
    title('Size = ', num2str(sizes(iSize)));
    axis square 
    refline(1);
  print(fullfile(fn_multi,['meanResp_',num2str(sizes(iSize)),'.pdf']),'-dpdf');

end

%% size tuning for each cell

[n n2] = subplotn(nCells);
figure; movegui('center');
for iCell = 1:nCells %for every size    
        subplot(n,n2,iCell)
        plot(sizes,mean(data_resp_match{pre}(iCell,:,:,1),3),'k');
        hold on
        plot(sizes,mean(data_resp_match{post}(iCell,:,:,1),3),'b');
        title(num2str(iCell))
        box off
end
...
figure;
subplot(1,2,1);
errorbar(sizes,size_green_avrg_stat{pre},size_green_se_stat{pre},'k');
hold on
errorbar(sizes,size_green_avrg_stat{post},size_green_se_stat{post},'b');
ylim([-.01 .02]);
title('-HTP, n = ', num2str(length(green_ind_match_list)));


subplot(1,2,2);
errorbar(sizes,size_red_avrg_stat{pre},size_green_se_stat{pre},'k');
hold on
errorbar(sizes,size_red_avrg_stat{post},size_green_se_stat{post},'b');
ylim([-.01 .02]);
title('-HTP, n = ', num2str(length(red_ind_match_list)));
...
for id = 1:nd
    ...
    resp_keep{id} = data_resp_match{id}(keep_cells,:,:,:,1);
...
for id = 1:nd
    ...
    for iSize = 1:nSize
        for iCell=1:nKeep
            ...
            temp_pref_responses_stat(iOri,iSize)=nanmean(nanmean(temp_TCs(stimStart:stimEnd,temp_trials_stat),1),2);
            temp_pref_responses_loc(iOri,iSize)=nanmean(nanmean(temp_TCs(stimStart:stimEnd,temp_trials_loc),1),2);
            temp_pref_responses_allCond(iOri,iSize)=nanmean(nanmean(temp_TCs(stimStart:stimEnd,temp_trials1),1),2);
            
            temp_tc_stat(:,iOri,iSize)=nanmean(temp_TCs(:,temp_trials_stat),2);
            temp_tc_loc(:,iOri,iSize)=nanmean(temp_TCs(:,temp_trials_loc),2);
            temp_tc_allCond(:,iOri,iSize)=nanmean(temp_TCs(:,temp_trials1),2);%average over all trials regardless of stationary vs. locomotion
...
rect_tc_trial_avrg_keep{1,iSize,id}=rect_tc_trial_avrg; %rectified version of above
...
for id = 1:nd
    ...
    for iOri=1:nKeep
            ...
            %identify the trials where ori = pref ori
            temp_ori= pref_ori_keep{id}(iOri); %find the preferred ori of this cell and convert to degrees
            ori_inds = find(tSize==temp_ori); %these are the trials at that ori
            inds=intersect(find(~RIx{id}),ori_inds);

            pref_responses_temp(iOri)=nanmean(nanmean(temp_TCs(stimStart:stimEnd,inds),1),2);
            pref_sd_temp(iOri)=std(nanmean(temp_TCs(stimStart:stimEnd,inds),1));
            tc_trial_avrg_temp(:,iOri)=nanmean(temp_TCs(:,inds),2);
...
for iOri = 1:length(red_ind_keep)
   red_keep_logical(red_ind_keep(iOri))=1;
end
...
explanation2 = 'data_resp_keep gives the df/f averaged over the full response window for all conditions in the form nCells X nOri X nCons X mean vs. std. resp_max_keep gives the df/f averaged over the full stim period for each cell at the preferred ori only, with a dimension for each contrast';
...
for iOri = 1:size(match_ind,2)
   ind = match_ind(iOri);
   temp_mask_inds = find(masks{1}==ind);
   mask_match{1}(temp_mask_inds)=iOri;
end
...
for iOri = 1:length(keep_cells)
  temp_mask_inds = find(keep_masks==iOri);
   if ismember(iOri,red_ind_keep)
       keep_red_masks(temp_mask_inds)=iOri;
       keep_masks_fract_change_red(temp_mask_inds) = dfof_max_diff(iOri,(size(dfof_max_diff,2)));
       keep_masks_raw_change_red(temp_mask_inds) = dfof_max_diff_raw(iOri,(size(dfof_max_diff,2)));
       keep_masks_d1_red(temp_mask_inds)=resp_max_keep{2}(iOri,size(resp_max_keep{2},2));
   else
       keep_green_masks(temp_mask_inds)=iOri;
       keep_masks_fract_change_green(temp_mask_inds) = dfof_max_diff(iOri,(size(dfof_max_diff,2)));
       keep_masks_raw_change_green(temp_mask_inds) = dfof_max_diff_raw(iOri,(size(dfof_max_diff,2)));
   end
end
...
for id = 1:nd
    for iSize=1:nCon
            locRectified = pref_responses_loc{id}(:,iSize);
            locRectified(find(locRectified<0))=0;
            statRectified = pref_responses_stat{id}(:,iSize);
            statRectified(find(statRectified<0))=0;
            LMI{id}(:,iSize)=(locRectified-statRectified)./(locRectified+statRectified);
...
for iSize = 1:nCon
    ...
    responseByCond(counter,:)=[mean(pref_responses_stat{pre}(green_ind_keep,iSize), "omitnan") mean(pref_responses_stat{pre}(red_ind_keep,iSize), "omitnan") mean(pref_responses_stat{post}(green_ind_keep,iSize), "omitnan") mean(pref_responses_stat{post}(red_ind_keep,iSize), "omitnan")]
    responseByCond((counter+1),:)=[mean(pref_responses_loc{pre}(green_ind_keep,iSize), "omitnan") mean(pref_responses_loc{pre}(red_ind_keep,iSize), "omitnan") mean(pref_responses_loc{post}(green_ind_keep,iSize), "omitnan") mean(pref_responses_loc{post}(red_ind_keep,iSize), "omitnan")]
...
for iOri=1:length(red_ind_keep)
    cellID=red_ind_keep(iOri);
    temp(iOri,:)=corrmat{id}(cellID,green_ind_keep);
end
green_corrs{id}=temp;
