...
day_id = 223; %enter post-DART day
...
for id = 1:nd
    ...
    for iSize = 1:nSize
        ...
        for iCon = 1:nCon
            ...
            data_resp(:,iSize,iCon,1) = squeeze(nanmean(nanmean(data_dfof_trial(resp_win,ind,:),1),2));
            data_resp(:,iSize,iCon,2) = squeeze(std(nanmean(data_dfof_trial(resp_win,ind,:),1),[],2)./sqrt(length(ind)));
            [h(:,iSize,iCon), p(:,iSize,iCon)] = ttest(nanmean(data_dfof_trial(resp_win,ind,:),1), nanmean(data_dfof_trial(base_win,ind,:),1),'dim',2,'tail','right','alpha',0.01./(nSize*nCon-1));
...
%             passThresh(:,iSize,iCon) = logical(data_resp(:,iSize,iCon,1) > thresh);
%             passThresh(:,iSize,iCon) = logical(data_resp(:,iSize,iCon,1) > .05);
%             h(:,iSize,iCon) = h(:,iSize,iCon) .*passThresh(:,iSize,iCon);
...
%     data_ori_resp = zeros(nCells,nSize); %at pref con
...
clear data_resp p h_all resp pref_ori pref_con data_ori_resp data_con_resp data_dfof_trial tCon tSize data_orth_resp  baseStd baseMean thresh pass
...
for iSize = 1:nSize
    figure;
    scatter(mean(data_resp_match{pre}(:,iSize,:,1),3),mean(data_resp_match{post}(:,iSize,:,1),3));
    axMax =  max(max(mean(data_resp_match{pre}(:,iSize,:,1),3),mean(data_resp_match{post}(:,iSize,:,1),3)));
    axMin =  min(min(mean(data_resp_match{pre}(:,iSize,:,1),3),mean(data_resp_match{post}(:,iSize,:,1),3)));
    xlim([axMin axMax]);
    ylim([axMin axMax]);
    xlabel('Pre');
    ylabel('Post');
...
meanCon = cell(1,nd);
for id = 1:nd
    meanCon{id}=mean(data_resp_match{id}(:,:,:,1),3);
end
