...
fprintf('Reference session: %s (position %d)\n', ref_session, pre);
frame_rate_double=double(frame_rate);
% Essential variables for running onset analysis:
% - cellTCs_match: neural timecourses for matched cells
% - wheel_speed_clean: cleaned wheel speed data
% - inputStructure: stimulus parameters and trial info
% - frame_rate: imaging frame rate
% - nOn, nOff: stimulus on/off durations
% - nTrials: number of trials per day
% - allDays: day identifiers
% - pre, post: indices for baseline and post-DART sessions
% - rc: directory structure
% - experimentFolder, mouse: for file paths
...
nRunOnsets = [];
meanSpeedAfterOnset = []; % NEW: Store mean speed after each onset
...
for id = 1:nd
    ...
    %make this variable - use photoFrameFinder if info.frame exists,
    %otherwise use the corrected input strucutre
    ...
    % Filter onsets to only include those during ITI
    ITIOnsets = [];
    for iOnset = 1:length(runOnsets_clean)
        onsetFrame = runOnsets_clean(iOnset);
        ...
        if onsetFrame <= length(ITI) && ...
           ...
           ITI(postOnsetFrame) == 1
            ITIOnsets = [ITIOnsets, onsetFrame];
        end
    end
    ...
    speedAfterOnset = nan(1, length(ITIOnsets));
    for iOnset = 1:length(ITIOnsets)
        onsetFrame = ITIOnsets(iOnset);
...
for id = 1:nd
    if ~isempty(meanSpeedAfterOnset{id})
        fprintf('Day %d: %d onsets, mean post-onset speed = %.2f cm/s\n', ...
            id, nRunOnsets(id), mean(meanSpeedAfterOnset{id}, 'omitmissing'));
    else
...
fn_out = fullfile(fn_multi,[num2str(stillLength),'sec'])
mkdir(fn_out)
save(fullfile(fn_out,'run_onset_analysis.mat'),'mean_resp_runOnset_match','data_dfof_runOnset_match','red_ind_match');
...
%         else
%             % Passed basic criteria, now check ITI
%             % Create ITI vector
%             ITI = ones(1, nFrames);
%             for iTrial = 1:inputStructure(id).trialSinceReset
%                 if cStimOn(iTrial) > 0 && cStimOn(iTrial) + inputStructure(id).nScansOn <= nFrames
...
%                ITI(preOnsetFrame) == 1 && ...
%                ITI(postOnsetFrame) == 1
%                 onsetStatus{iOnset} = 'kept';
%             else
%                 onsetStatus{iOnset} = 'outside_ITI';
%             end
%         end
%     end
% 
...
% %%
% % 1) Plot wheel speed with ALL onsets color-coded by rejection reason
% figure('Position', [100, 100, 1000, 600]);
% 
% % Define colors for different rejection reasons
% colorMap = containers.Map();
% colorMap('kept') = [0, 0.8, 0]; % Green
...
% colorMap('outside_ITI') = [1, 1, 0]; % Yellow
% 
% for id = 1:nd
%     subplot(nd, 1, id);
% 
...
% end
% 
% sgtitle('All Detected Onsets Color-Coded by Filtering Result');
% 
...
% 
% % Save table to file
% writetable(onsetSummaryTable, 'onset_filtering_summary.csv');
% save('onset_filtering_summary.mat', 'onsetSummaryTable');
