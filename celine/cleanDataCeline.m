...
p.addParamValue('afterStimS', 1, @isnumeric);
...
if params.pclamp
    ...
    if params.rebaseline
        if params.slice
            ...
            if abs(mean(new_data.pts(500,trial))) > 20
                new_data.pts(:,trial) = new_data.pts(:,trial) - mean(new_data.pts(200:700,trial));
            end
        end
    ...
    if params.rebaseline
        if any(abs(mean(new_data.pts(550:650,:))) > 50)
            replace_i = abs(mean(new_data.pts(550:650,:))) > 50;
            new_data.pts(:,replace_i) = new_data.pts(:,replace_i) - mean(new_data.pts(200:600,replace_i),'omitnan');
        end
