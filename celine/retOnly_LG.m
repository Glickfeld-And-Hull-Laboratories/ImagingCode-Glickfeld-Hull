...
elseif doFromRef
    ...
    load(fullfile(fnOut, [date '_' mouse '_' ref_str '_reg_shifts.mat']))
    load(fullfile(fnOut, [date '_' mouse '_' ref_str '_mask_cell.mat']))
...
if input.doRetStim
    ...
    % print to pdf
    print(fullfile(fnOut, [date '_' mouse '_' run_str '_FOV_resp_Ret.pdf']), '-dpdf')
...
save(fullfile(fnOut, [date '_' mouse '_' run_str '_stimActFOV.mat']), 'data_dfof_max', 'data_dfof_avg', 'nStim')
...
if doRed
    save(fullfile(fnOut, [date '_' mouse '_' run_str '_mask_cell.mat']), 'mask_cell_red','mask_label','mask_cell', 'mask_np', '-v7.3')
else
    save(fullfile(fnOut, [date '_' mouse '_' run_str '_mask_cell.mat']),'mask_cell', 'mask_np', '-v7.3')
end
...
load(fullfile(fnOut, [date '_' mouse '_' run_str '_mask_cell.mat']))
...
set(gcf, 'Position', [0 0 800 1000]);
print(fullfile(fnOut, [date '_' mouse '_' run_str '_FOVresp.pdf']), '-dpdf')
save(fullfile(fnOut, [date '_' mouse '_' run_str '_FOVs.mat']), 'data_dfof_avg', 'Azs', 'Els','Stims')
...
set(gcf, 'Position', [0 0 800 1000]);
print(fullfile(fnOut, [date '_' mouse '_' run_str '_Tuning' num2str(f) '.pdf']), '-dpdf')
save(fullfile(fnOut, [date '_' mouse '_' run_str '_Tuning.mat']), 'tc_dfof', 'tuning_mat', 'Stims', 'Ind_struct')
...
for count_shuf = 0:Nshuf
    ...
    if count_shuf == 0  
        set(gcf, 'Position', [0 0 800 1000]);
        fn_out = fullfile(fnOut, [date '_' mouse '_' run_str '_RFfits' num2str(ifig) '.pdf']);
        print(fn_out,'-dpdf')
    end
end
fprintf('Shuffling done, saving fit results\n')

fn_out = fullfile(fnOut, [date '_' mouse '_' run_str '_Fit_struct.mat']);
...
fn_out = fullfile(fnOut, [date '_' mouse '_' run_str '_lbub_fits.mat']);
...
set(gcf, 'Position', [0 0 800 1000]);
fn_out = fullfile(fnOut, [date '_' mouse '_' run_str '_RFs.pdf']);
print(fn_out,'-dpdf')
...
ylabel('Peak dF/F')
set(gcf, 'Position', [0 0 800 1000]);
fn_out = fullfile(fnOut, [date '_' mouse '_' run_str '_RFdists.pdf']);
print(fn_out,'-dpdf')
...
print(fullfile(fnOut, [date '_' mouse '_' run_str '_retMap.pdf']), '-dpdf')
...
load(fullfile(fnOut, [date '_' mouse '_' run_str '_FOVs.mat']))
% _Tuning.mat: 'tc_dfof', 'tuning_mat', 'Stims', 'Ind_struct'
load(fullfile(fnOut, [date '_' mouse '_' run_str '_Tuning.mat']))
...
for iCell = 1:size(tuning_mat,3)
    if max(tuning_mat(:,1,iCell)) < 0.05
        ...
        if sp >36
            set(gcf, 'Position', [0 0 800 1000]);
            print(fullfile(fnOut, [date '_' mouse '_' run_str '_RawFit' num2str(f) '.pdf']), '-dpdf', '-bestfit')
            sp = 1;
            f = f+1;
            figure;
        end
