...
% compute R2
ss_res = sum((s.data(:) - s.k2b(:)).^2);
ss_tot = sum((s.data(:) - mean(s.data(:))).^2);
...
if PLOTIT_FIT == 1
    ...
    if s.r2 >= r2_thresh; pass_str = ' OK'; else; pass_str = ' x'; end
    if h_all(1,iCell);    sig_str  = '**';  else; sig_str  = [];   end
    ttl = [num2str(iCell) ' ' sig_str ' R2=' num2str(s.r2,2) pass_str];

    h = subplot(4,8, start);
    imagesc(s.data); colormap('gray'); axis image off;
    title(ttl, 'FontSize', 7);

    h = subplot(4,8, start+1);
