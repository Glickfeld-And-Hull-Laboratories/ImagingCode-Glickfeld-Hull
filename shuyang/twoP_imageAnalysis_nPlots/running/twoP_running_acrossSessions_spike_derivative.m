...
for ii = 1: length(sessions)
    image_analysis_dest = [image_analysis_base, sessions{ii}, '\derivative\'];
    spk_output = load([image_analysis_dest sessions{ii} '_spikes.mat']);
    FRstay_cell_cl = spk_output.FRstay_cells;
    FRrun_cell = spk_output.FRrun_cells;
...
path = 'Z:\Analysis\2P_MovingDots_Analysis\across_sessions\derivative\';
...
for ii = 1:length(sessions) 
    %load things
    image_analysis_dest = [image_analysis_base, sessions{ii}, '\derivative\'];
    spike_output = load([image_analysis_dest sessions{ii} '_spikes.mat']);
