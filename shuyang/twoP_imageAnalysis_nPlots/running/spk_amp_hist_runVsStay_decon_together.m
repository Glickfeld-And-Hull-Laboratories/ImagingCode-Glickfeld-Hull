% histogram of detected CS peaks during running and stationary
...
for ii = 4
    ...
    %spk_deconv_output = load([image_analysis_dest '\deconvolution_staywRunSmin\' sessions{ii},'_spk_deconvolve_staynrun_seperate' '.mat']);
    spk_inx_cl = spk_deconv_output.spk_inx_cl;
    %spk_inx_cl = spk_deconv_output.spk_inx;
    peaks_run = [];
    peaks_stay = [];
    ...
    set(gca,'XTickLabel',a,'FontSize',7);
    hold off;
    %saveas(hist_fig,[image_analysis_dest 'deconvolution\' sessions{ii} '_SpkeventAmp_hist']);
    %saveas(hist_fig,[image_analysis_dest 'deconvolution_staywRunSmin\' sessions{ii} '_SpkeventAmp_hist_sep']);
    fig_name = [sessions{ii} '_SpkeventAmp_hist_deconv_together'];
    %fig_name = [sessions{ii} '_SpkeventAmp_hist_sep'];
    path = [image_analysis_dest 'deconvolution\'];
    %path = [image_analysis_dest 'deconvolution_staywRunSmin\'];
