...
text(1.2, 0.3, ['n = ' num2str(nPCs_total)],'FontSize', 20,'FontName','Arial');
ylabel('Cspk rate (Hz)');
...
%data_file = 'across_sessions_RC_data_CS1CS2_testCS1CS2_lickAlign.mat';
%data_file = 'across_sessions_RC_data_CS1CS2_testCS1_lickAlign.mat';
...
%title('CS1+CS2 trained,  test w/CS1+CS2, align to first lick post reward, across sessions');
%title('CS1+CS2 trained, test w/CS2,  align to first lick post reward, across sessions');
%title('CS1+CS2 trained, test w/CS1,  align to first lick post reward, across sessions');
...
%fig_name = 'across_session_firstLickAlign_CS1CS2_testCS1CS2';
fig_name = 'across_session_firstLickAlign_CS1CS2_testCS1';
...
lickBurstAlign_fig.Position = [1 1 4 3];
subplot(2,1,1);
...
neural_by_lick_time.Units = 'centimeters';
neural_by_lick_time.Position = [1 1 12 8];
shadedErrorBar(tt/1000,nanmean(targetAlign_events_earlybst,2).*(1000./frameRateHz), (nanstd(targetAlign_events_earlybst,0,2)).*(1000./frameRateHz)./sqrt(nPCs_total), 'k');
hold on;
shadedErrorBar(tt/1000,nanmean(targetAlign_events_latebst,2).*(1000./frameRateHz), (nanstd(targetAlign_events_latebst,0,2)).*(1000./frameRateHz)./sqrt(nPCs_total), 'b');
errorbar((mean(earlyBst_sessions))/1000,2,(std(earlyBst_sessions))/sqrt(size(earlyBst_sessions,2))/1000,'horizontal','-s','LineStyle','none','MarkerFaceColor','k','MarkerEdgeColor','k','Color','k');
errorbar((mean(lateBst_sessions))/1000,2,(std(lateBst_sessions))/sqrt(size(earlyBst_sessions,2))/1000,'horizontal','-s','LineStyle','none','MarkerFaceColor','b','MarkerEdgeColor','b','Color','b');
ylabel('Cspk rate (Hz)');
xlabel('time relative to cue (s)');
ylim([0 3]); xlim([-1 2]);
vline(0,color_code);vline(0.7,'k');
a = get(gca,'XTickLabel');
...
neural_by_lick_rate.Units = 'centimeters';
neural_by_lick_rate.Position = [1 1 12 8];
shadedErrorBar(tt/1000,nanmean(targetAlign_events_lowlick_postrew,2).*(1000./frameRateHz), (nanstd(targetAlign_events_lowlick_postrew,0,2)).*(1000./frameRateHz)./sqrt(nPCs_total), 'b');
hold on;
shadedErrorBar(tt/1000,nanmean(targetAlign_events_highlick_postrew,2).*(1000./frameRateHz), (nanstd(targetAlign_events_highlick_postrew,0,2)).*(1000./frameRateHz)./sqrt(nPCs_total), 'k');
ylabel('Cspk rate (Hz)');
xlabel('time relative to cue (s)');
ylim([0 3]); xlim([-1 2])
vline(0,color_code);vline(0.7,'k');
...
neural_by_piezo.Units = 'centimeters';
neural_by_piezo.Position = [1 1 12 15];
subplot(2,1,1);
shadedErrorBar(tt/1000,nanmean(lessmove_prerew_sessions,2).*(1000./frameRateHz), (nanstd(lessmove_prerew_sessions,0,2)).*(1000./frameRateHz)./sqrt(size(lessmove_prerew_sessions,2)), 'b');
hold on;
shadedErrorBar(tt/1000,nanmean(mostmove_prerew_sessions,2).*(1000./frameRateHz), (nanstd(mostmove_prerew_sessions,0,2)).*(1000./frameRateHz)./sqrt(size(lessmove_prerew_sessions,2)), 'k');
ylabel('Piezo (V)');
ylim([0 10]);xlim([-1 2]);
vline(0,color_code);vline(0.7,'k');
...
set(gca,'XTickLabel',a,'FontSize',20);
subplot(2,1,2);
shadedErrorBar(tt/1000,nanmean(targetAlign_events_lessmove_prerew,2).*(1000./frameRateHz), (nanstd(targetAlign_events_lessmove_prerew,0,2)).*(1000./frameRateHz)./sqrt(nPCs_total), 'b');
hold on;
shadedErrorBar(tt/1000,nanmean(targetAlign_events_mostmove_prerew,2).*(1000./frameRateHz), (nanstd(targetAlign_events_mostmove_prerew,0,2)).*(1000./frameRateHz)./sqrt(nPCs_total), 'k');
ylabel('Cspk rate (Hz)');
xlabel('time relative to cue (s)');
ylim([0 3]); xlim([-1 2]);
vline(0,color_code);vline(0.7,'k');
...
for ii = 1:length(sessions) 
    ...
    ntrials_total = ntrials_total + size(targetAlign_events_thisSession,3);
    nPCs = cat(2,nPCs,size(targetAlign_events_thisSession,2));
    trailTypes = load (['Z:\home\shuyang\behavior_analysis\RC\' sessions{ii} '_IL_trial_inx.mat']);
...
targetAlign_interleave_fig.Position = [1 1 4 4];
a = get(gca,'XTickLabel');
set(gca,'XTickLabel',a,'FontSize',10);
subplot(3,1,1);
