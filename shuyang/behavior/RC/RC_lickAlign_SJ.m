...
for  j = 1:length(expt.ttl)
    ...
    cTargetOn(1) = [];% first trial doesn't have reward in CS1 training
    ...
    for itrial = 1:nTrials
        if cTargetOn(itrial)+postwin_frames-1 < nFrames
            lickTimes = input.lickometerTimesUs{itrial+1}; % needs to do itrial+1 here because there's no juice in the first trial, already deleted cTargetOn(1), so the first cTargetOn should be aligned with the second cell in lickometerTimes
            counterTimes = input.counterTimesUs{itrial+1};
            counterVals = input.counterValues{itrial+1};
            ...
            if input.counterValues{itrial+1}(end)-cTargetOn(itrial) > postwin_frames
    ...
    if sum(~isnan(lickBurstStart))>6
        ...
        early_rew_time = mean((lickBurstStart(:,ind_early_rew)-prewin_frames).*(1000./frameRateHz),2);
        late_rew_time = mean((lickBurstStart(:,ind_late_rew)-prewin_frames).*(1000./frameRateHz),2);
    else
        ind_early_rew = [];
        ind_late_rew = [];
        early_rew_time = [];
    ...
    if sum(~isnan(lickBurstStart))>6
        figure;
        shadedErrorBar(tt,nanmean(nanmean(targetAlign_events(:,:,ind_early_rew),3),2).*(1000./frameRateHz), (nanstd(nanmean(targetAlign_events(:,:,ind_early_rew),3),[],2).*(1000./frameRateHz))./sqrt(nIC), 'k');
        hold on;
        shadedErrorBar(tt,nanmean(nanmean(targetAlign_events(:,:,ind_late_rew),3),2).*(1000./frameRateHz), (nanstd(nanmean(targetAlign_events(:,:,ind_late_rew),3),[],2).*(1000./frameRateHz))./sqrt(nIC),'b');
        scatter((lickBurstStart(:,ind_early_rew)-prewin_frames).*(1000./frameRateHz),-0.5.*ones(1,length(ind_early_rew)),'xk');
        scatter((lickBurstStart(:,ind_late_rew)-prewin_frames).*(1000./frameRateHz),-0.5.*ones(1,length(ind_late_rew)),'xb');
        xlabel('Time from cue');
    ...
    early_rew_time = nanmean((postRew_lickBurstStart(:,ind_prerew_early_rew)-prewin_frames-rewDelay_frames).*(1000./frameRateHz),2);
    late_rew_time = nanmean((postRew_lickBurstStart(:,ind_prerew_late_rew)-prewin_frames-rewDelay_frames).*(1000./frameRateHz),2);
    shadedErrorBar(tl, nanmean(nanmean(postRew_lickAlignEvents(:,:,ind_prerew_early_rew),3),2).*(1000./frameRateHz), (nanstd(nanmean(postRew_lickAlignEvents(:,:,ind_prerew_early_rew),3),[],2).*(1000./frameRateHz))./sqrt(nIC),'k');
    hold on;
    shadedErrorBar(tl, nanmean(nanmean(postRew_lickAlignEvents(:,:,ind_prerew_late_rew),3),2).*(1000./frameRateHz), (nanstd(nanmean(postRew_lickAlignEvents(:,:,ind_prerew_late_rew),3),[],2).*(1000./frameRateHz))./sqrt(nIC),'b');
    xlabel('Time from lick');
    ylabel('Spike rate (Hz)');
    ylim([0 inf]);
    ...
    ylim([0 inf]);
    subplot(2,2,4);
    shadedErrorBar(tl, nanmean(postRew_lickAlign(:,ind_prerew_early_rew),2).*(1000./frameRateHz), (nanstd(postRew_lickAlign(:,ind_prerew_early_rew),[],2).*(1000./frameRateHz))./sqrt(length(ind_prerew_early_rew)),'k');
    hold on;
    shadedErrorBar(tl, nanmean(postRew_lickAlign(:,ind_prerew_late_rew),2).*(1000./frameRateHz), (nanstd(postRew_lickAlign(:,ind_prerew_late_rew),[],2).*(1000./frameRateHz))./sqrt(length(ind_prerew_late_rew)),'b');
    xlabel('Time from lick');
    ylabel('Lick rate (Hz)');
    ylim([0 inf]);
    ...
    ind_low_rew = sortlickHz_ind(1:floor(nburst/4));
    ind_high_rew = sortlickHz_ind(nburst-floor(nburst/4)+1:end-nnan);
    HL_lickrate.low_rew = mean(lickBurstHz_all(:,ind_low_rew),2);
    HL_lickrate.high_rew = mean(lickBurstHz_all(:,ind_high_rew),2);
    ...
    figure;
    subplot(1,3,1);
    shadedErrorBar(tt,nanmean(nanmean(targetAlign_events(:,:,ind_low_rew),3),2).*(1000./frameRateHz), (nanstd(nanmean(targetAlign_events(:,:,ind_low_rew),3),[],2).*(1000./frameRateHz))./sqrt(nIC), 'b');
    hold on;
    shadedErrorBar(tt,nanmean(nanmean(targetAlign_events(:,:,ind_high_rew),3),2).*(1000./frameRateHz), (nanstd(nanmean(targetAlign_events(:,:,ind_high_rew),3),[],2).*(1000./frameRateHz))./sqrt(nIC),'k');
    xlabel('Time from cue');
    ylabel('Spike rate (Hz)');
    ylim([-1 inf]);
    title(['1s- Rew: ' num2str(chop(HL_lickrate.low_rew,2)) ' vs ' num2str(chop(HL_lickrate.high_rew,2)) ' Hz']);
    subplot(1,3,2);
    shadedErrorBar(tt,nanmean(nanmean(targetAlign_events(:,:,ind_low_prerew),3),2).*(1000./frameRateHz), (nanstd(nanmean(targetAlign_events(:,:,ind_low_prerew),3),[],2).*(1000./frameRateHz))./sqrt(nIC), 'b');
    ...
    subplot(1,3,3);
    shadedErrorBar(tt,nanmean(nanmean(targetAlign_events(:,:,ind_low_postrew),3),2).*(1000./frameRateHz), (nanstd(nanmean(targetAlign_events(:,:,ind_low_postrew),3),[],2).*(1000./frameRateHz))./sqrt(nIC), 'b');
    ...
    save(fullfile(analysis_out,img_fn, [img_fn '_' run '_cueAlignLick.mat']), 'firstPostRewLickFrame', ...
        ...
        'lickTC', 'postwin_frames', 'prewin_frames', 'frameRateHz', 'tt', 'ind_early_rew', 'ind_late_rew', ...
        'early_rew_time', 'late_rew_time', 'pct_precue_burst', 'postRew_lickAlignEvents', 'postLick_frames', ...
        'postRew_lickBurstStart','tl', 'ind_prerew_early_rew', 'ind_prerew_late_rew','postRew_lickAlign',...
        'preRew_lickBurstHz','postRew_lickBurstHz','ind_low_prerew','ind_high_prerew','ind_low_postrew',...
        'ind_high_postrew','ind_high_rew','ind_low_rew','HL_lickrate');
