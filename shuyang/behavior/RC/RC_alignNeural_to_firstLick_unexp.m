...
for  j = 1:length(expt.ttl)
    ...
    box off;
    subplot(2,2,2);
    shadedErrorBar(tt, nanmean(nanmean(postRew_lickAlignEvents_1500_3000ms_scale(:,:,unexptrials),3),2).*(1000./frameRateHz), (nanstd(nanmean(postRew_lickAlignEvents_1500_3000ms_scale(:,:,unexptrials),3),[],2).*(1000./frameRateHz))./sqrt(nIC),'k');
    ylim([0 4]);vline(0,'k');ylabel('spike rate (Hz)');
    title('unexpected');box off;
    subplot(2,2,4);
    shadedErrorBar(tt, nanmean(postRew_lickAlign_1500_3000ms_scale(:,unexptrials),2).*(1000./frameRateHz), (nanstd(postRew_lickAlign_1500_3000ms_scale(:,unexptrials),[],2).*(1000./frameRateHz))./sqrt(nIC),'k');
    ylim([0 40]);vline(0,'k');ylabel('lick rate (Hz)');xlabel('time from lick burst (ms)');
    box off;
    ...
    box off;
    subplot(2,2,2);
    shadedErrorBar(tt, nanmean(nanmean(firstPostRew_lickAlignEvents_1500_3000ms_scale(:,:,unexptrials),3),2).*(1000./frameRateHz), (nanstd(nanmean(firstPostRew_lickAlignEvents_1500_3000ms_scale(:,:,unexptrials),3),[],2).*(1000./frameRateHz))./sqrt(nIC),'k');
    ylim([0 4]);vline(0,'k');ylabel('spike rate (Hz)');
    title('unexpected');box off;
    subplot(2,2,4);
    shadedErrorBar(tt, nanmean(nanmean(firstPostRew_lickAlign_1500_3000ms_scale(:,unexptrials),3),2).*(1000./frameRateHz), (nanstd(nanmean(firstPostRew_lickAlign_1500_3000ms_scale(:,unexptrials),3),[],2).*(1000./frameRateHz))./sqrt(nIC),'k');
    ylim([0 40]);vline(0,'k');ylabel('lick rate (Hz)');xlabel('time from first lick (ms)');
    box off;
