...
clear;
bdata_source = 'Z:\Data\behavior\auditory_RC\';
analysis_outputs_dir = 'Z:\behavior_analysis\RC_auditory\';
CRP_fig_dir_base = 'Z:\behavior_analysis\RC_auditory\sessions_&_summaries\';

time_before_ms = 2000; %defines the window around the cue presentation which will be taken for plotting
time_after_ms = 3000;

CRP_training_days_lists_SJ;
...
for ii = 1:length(days)
    days{ii}
    licktimes_dir  = [analysis_outputs_dir days{ii} '_licktimes'];
    b_data = get_bx_data_sj(bdata_source, days{ii});  %find the correct behavior file and loads it.
    [this_mouse, this_date] = select_mouse_info(days{ii}); % gets the mouse number and date of the experiment
    this_sess = days{ii};
...
%{ 
 ...
 %}
    ...
    save(['Z:\behavior_analysis\RC_auditory\RT_data\', days{ii}], 'RT_this_session_raw', 'time_before_ms');  
...
%         end
    ...
    %determine trial index for trials on different days and non-consecutive days
...
%         end
    ...
    save(['Z:\behavior_analysis\RC_auditory\hist_data_across_animals\', days{ii}, 'rew_hist'], 'full_trial_licks_rewarded_bin','x_axis_bin');
...
%         end
%         save(['Z:\behavior_analysis\RC_auditory\hist_data_across_animals\', days{ii}, 'rew_om_hist'], 'full_trial_licks_omit_bin');
%     end
...
%         end
%         save(['Z:\behavior_analysis\RC_auditory\hist_data_across_animals\', days{ii}, 'block2_hist'], 'full_trial_licks_block2_bin');
%     end
...
save(['Z:\behavior_analysis\RC_auditory\post_cue_lick_across_animals\', days{ii}(end-4:end), 'rew_om_post_cue_lick'], ...
...
if jakes_RT_filtering
save(['Z:\behavior_analysis\RC_auditory\RT_stats_across_animals\', days{ii}(end-4:end), 'RTstd_FA_misses'], ...
...
out_dir = 'Z:\behavior_analysis\RC_auditory\RT_across_animals\';
